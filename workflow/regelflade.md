...
**S3 — worklog/ledger + drift-gate ✅ bygget:** `workflow/worklog.json` (state) · skema `workflow/worklog.schema.json` · checker `scripts/workflow/worklog-check.mjs` · bevis `scripts/workflow/worklog-check.selftest.mjs`. Worklog er den **pålidelige state-kilde** (j)'s `currentFromState` binder mod (Codex' afhængighed): det mekaniske felt `kravHash` gen-beregnes fra krav-dokket — hand-edit der lyver → **DRIFT BLOKERET**. **Gate-state (`kravOK`/`planOK`/`buildOK`) lever her**, ikke i den hash-bundne krav-body — løser UDKAST-linje-problemet strukturelt. **`gateRecord`** er den eksplicitte bogføring (hvem gatede på hvilken hash): en gatet worklog SKAL bære den (`manglerGateRecord`), og den kan ikke lyve om hashen (`gateRecordLoegn`) — så main bærer en utvetydig, checket "krav OK c964826 / plan OK 94c70eb / build OK 97a650d (Mathias)"-record uden at røre den immutable krav-body (Claude.ai-fund 2, lukket ved bogføring). `buildOK`-record'en binder til `mergeSha` (main-@-hash efter #168-merge, merge-hash-flow) — det fulde gate-spor krav→plan→build er dermed bogført på main. **Drift-gaten fanger løgn i begge** (Codex' checkliste): `kravHash` gen-beregnes fra krav-dok (hård); `planSha`-format + **HÅRD git-eksistens** — henter den autoritative `planRef` (plan-branchen) FØRST hvis objektet mangler i checkout'en, og afviser hvis SHA'en stadig ikke findes (fabrikeret-men-format-gyldig SHA → FAIL, ingen warning-snyd; Codex-lukning); gate-state konsistens-tjekkes (planOK uden kravOK/planSha, buildOK uden planOK → DRIFT). Net: worklog er **kilde-verificeret** (kan ikke blive en lyvende manuel sandhed ved siden af git/artefakter — point 1/4), ikke bare hand-skrevet. (Fuld auto-generering af gate-state fra committede gate-records = forward, Pakke 2.) Real-worklog-drift køres som CI-step (`workflow:selftest`).
