#!/usr/bin/env node
// build-proof.selftest.mjs — red-team af verifyBuildProof v2.3 (C1-r2 F-11..19 · C1-r3 F-20..27): manifest OG angrebs-spec er gate-bindinger;
...
const MANIFEST = { schema_version: 1, pakke: "pakke-x",
  ...
  guards: [{ id: "g.navn", beskrivelse: "navn" }, { id: "g.min", beskrivelse: "min" }, { id: "g.klass", beskrivelse: "klass" }, { id: "g.laas", beskrivelse: "række-lås (SA-overlap)" }],
  obligations: [
...
expectRed("claim: source_anchor peger på gyldigt git-uddrag der IKKE nævner routinen (plan.md) → rød (F-25)", verify(mutated((p) => (p.claim_graph[0].source_anchor = mkEvidence("plan/plan.md", 1, 1)))), "nævner ikke 'lokation_opret'");
expectRed("claim: source_anchor på migrationens ANDEN funktion (stand_deaktiver) → rød (F-25)", verify(mutated((p) => (p.claim_graph[0].source_anchor = mkEvidence("supabase/migrations/0001.sql", 2, 2)))), "nævner ikke 'lokation_opret'");
