#!/usr/bin/env node
// build-proof.selftest.mjs — red-team af verifyBuildProof v2.2 (C1-r2 F-11..F-19): manifest OG angrebs-spec er gate-bindinger;
...
const MANIFEST = { schema_version: 1, pakke: "pakke-x",
  ...
  guards: [{ id: "g.navn", beskrivelse: "navn" }, { id: "g.min", beskrivelse: "min" }, { id: "g.klass", beskrivelse: "klass" }],
  obligations: [
...
function mkRunner() {
  ...
  return { st,
    sql(t) { switch (t) {
      ...
      case "FP": return R(true, null, null, null, [{ navn: st.navn, trg: st.trg, klass: st.klass }]);
...
const greenProof = () => ({
  ...
  prover_result: { ok: true, total: 9, passed: 9, failed: 0, skipped: 0, executed_ids: [...ALL_IDS] },
});
...
console.log("verifyBuildProof v2.2 — grøn sti (bevis produceret af motoren mod den gate-bundne spec):");
expectGreen("ægte v2.2-bevis → grøn", verify(greenProof()));
...
expectRed("prover_result uden konsistent resumé (failed 99 skjult) → rød", verify(mutated((p) => (p.prover_result = { ok: true, total: 9, passed: 9, failed: 99, skipped: 0, executed_ids: ALL_IDS }))), "prover_result");
...
{ const r = evaluateGate("build", snap(greenProof()), { verifyProof: makeProofVerifier({ git }) }); r.open ? ok("build-gaten ÅBNER med ægte v2.2-bevis + kæde-bundet manifest + gate-bundet spec") : bad("e2e-grøn", r.reasons.join(" | ")); }
