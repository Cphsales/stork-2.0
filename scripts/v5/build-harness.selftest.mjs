...
eq("runner leverer ok:true m. detail.message som TAL (42601) → protokol, repareres ikke til null (F-37)", await st(C.ut, mkRunner({ sql: (t) => (t === "STATE" ? { ok: true, code: null, error: null, detail: { message: 42601, routine: null }, rows: [{ n: 1 }] } : undefined) })), STATUS.PROTOKOL);
eq("runner leverer ok:true m. code:'' (tom streng) på setup → protokol, forsvinder ikke i projektionen (F-37)", await st(withCase(C.ut, (x) => (x.setup = { sql: "SETUP" })), mkRunner({ sql: (t) => (t === "SETUP" ? { ok: true, code: "", error: null, detail: null } : undefined) })), STATUS.PROTOKOL);
eq("runner leverer ok:true m. detail.routine som streng på observe → protokol (F-37)", await st(C.fs, mkRunner({ sql: (t) => (t === "OBS" ? { ok: true, code: null, error: null, detail: { message: null, routine: "f.x" }, rows: [{ pris: 100 }] } : undefined) })), STATUS.PROTOKOL);
eq("SA: session A leverer ok:true m. detail:'syntax error' (streng) → protokol, sess() reparerer ikke (F-37)", await st(C.sa, mkRunner({ race: () => ({ protocolOk: true, a: { pid: 1, ok: true, code: null, detail: "syntax error", commit: "commit" }, b: { pid: 2, ok: false, code: "P0001", detail: { message: "min_en_stand", routine: "f.stand_deaktiver" }, commit: "rollback" }, overlap: { observed: true, witness_pid: 303, a_pid: 1, b_pid: 2 }, invariantRows: [{ aktive: 1 }] }) })), STATUS.PROTOKOL);
eq("SA: session A leverer detail:[] → protokol (F-37)", await st(C.sa, mkRunner({ race: () => ({ protocolOk: true, a: { pid: 1, ok: true, code: null, detail: [], commit: "commit" }, b: { pid: 2, ok: false, code: "P0001", detail: { message: "min_en_stand", routine: "f.stand_deaktiver" }, commit: "rollback" }, overlap: { observed: true, witness_pid: 303, a_pid: 1, b_pid: 2 }, invariantRows: [{ aktive: 1 }] }) })), STATUS.PROTOKOL);
eq("SA: session B afvist m. detail.message som tal → protokol (F-37)", await st(C.sa, mkRunner({ race: () => ({ protocolOk: true, a: { pid: 1, ok: true, code: null, detail: null, commit: "commit" }, b: { pid: 2, ok: false, code: "P0001", detail: { message: 42, routine: "f.stand_deaktiver" }, commit: "rollback" }, overlap: { observed: true, witness_pid: 303, a_pid: 1, b_pid: 2 }, invariantRows: [{ aktive: 1 }] }) })), STATUS.PROTOKOL);
...
{ const r = await kill(M.sa, mkRunner({ race: (s, st) => ({ protocolOk: true, a: { pid: 1, ok: true, code: null, detail: st.trg ? null : "syntax error", commit: "commit" }, b: st.trg ? { pid: 2, ok: false, code: "P0001", detail: { message: "min_en_stand", routine: "f.stand_deaktiver" }, commit: "rollback" } : { pid: 2, ok: true, code: null, detail: null, commit: "commit" }, overlap: { observed: true, witness_pid: 303, a_pid: 1, b_pid: 2 }, invariantRows: [{ aktive: st.trg ? 1 : 0 }] }) })); eq("SA-mutant: session A m. ugyldigt rå-detail under mutanten → protokol → ikke dræbt (F-37)", r.killed === false && r.under?.status === STATUS.PROTOKOL, true); }
