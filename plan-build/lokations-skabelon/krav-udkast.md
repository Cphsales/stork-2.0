...
- **Struktur (umuligt):** cykler (en lokation kan aldrig — direkte eller gennem led — være sin egen over-lokation) · stand uden entydig pris-opløsning · to klienter på samme stand samtidig · booking på stand under ikke-aktiv lokation (via K-4).
...
8. At koble en klient på eller fra en nedlagt lokation AFVISES — lokationen skal være åben først (Mathias 2026-09-03: "den skal være åben", bekræftet "nej"); genåbn (ac 7), kobl derefter. Eksisterende koblinger og deres historik består uændret (K-6 ac 7).
...
**HVAD:** Stork styrer hvilke klienter der må stå og sælges på hvilke lokationer (forretningsforstaaelse §14) — systemet håndhæver det, aldrig en manuel konvention. Klienter kobles på og fra de enkelte lokationer (K-3; Mathias 2/9: pr. lokation, ikke pr. kæde · 2026-09-03: "klienter skal altid kunne kobles på og fra på de enkelte lokationer") — manuelt, af rettighedshavere; ingen automatik kobler en klient af. En kobling gælder fra den sættes indtil klienten aktivt kobles fra — ingen slutdato kræves (Mathias 2026-09-03: "lokationer har ingen slutdato før vi deaktivere den/nedlægger den"). Lokationens status styrer kun bookbarhed (K-4), ikke koblingerne. Flere klienter kan have tilladelse på samme lokation samtidig; grænsen er standen (K-2): én klient pr. stand ad gangen, og hvilken stand klienten står på afgøres ved booking (trin 24). Fortiden bevares: hvad der gjaldt hvornår kan altid ses (princip 9). Historik bevares altid (Mathias 2026-09-03: "vigtigt vi bevarer den historiske data"). *Plan-mekanik (ikke krav-sprog):* gældende-dato, versionering og fortrydelses-mekanismen (§1.7) følger fundamentets mønster for daterede ændringer.

- **Struktur (umuligt):** tilladelse uden klient eller lokation · to gældende tilladelser for samme klient×lokation (én sandhed) · kæde-tilladelse (K-3) · til-/frakobling på nedlagt lokation (K-4) · automatisk frakobling ved status-skift · sletning/omskrivning af historik · booking af klient uden gældende tilladelse på lokationen på bookingdatoen (håndhævet i skrivevejen — trin 24 forbruger opslags-fladen).
...
3. Ingen slutdato kræves: en kobling uden slutdato AFVISES IKKE; den gælder indtil klienten aktivt kobles fra (Mathias 2026-09-03).
...
7. Nedlæggelse eller andet status-skift på lokationen kobler INGEN klient af og sletter ingen historik — koblingerne og deres historik består (Mathias 2026-09-03: "altid kunne kobles på og fra" · "vigtigt vi bevarer den historiske data"). Mens lokationen er nedlagt kan koblinger ikke ændres (K-4 ac 8); efter genåbning kobles der på og fra igen.
...
| intet-data:klient-tilladelse-kommerciel-betydning | 3 | behandlet | K-6 (Mathias 2/9: pr. lokation, gruppe ejer — K-3 · 3/9: flere klienter pr. lokation, én pr. stand · kobles på/fra manuelt, ingen slutdato, ingen automatisk frakobling · nedlagt: til-/frakobling afvises, åben først (K-4 ac 8), historik består · eksisterende bookinger ved frakobling → trin 24) |
