...
// Kolonnerne er læst i supabase/migrations @ 79ab859 (CREATE TABLE + ALLE senere ADD COLUMN, også på fortsættelseslinjer — v3 efter Codex A4-1: fire kolonner på anonymization_mappings var overset) — ingen kolonne er antaget.
...
const TABELLER = [
  ...
  { skema: "core_compliance", rel: "anonymization_mappings", art: "closure", ddl: "20260514140000_t6_anonymization_tables.sql:19 + c002:37-41 (internal_rpc_anonymize · internal_rpc_apply · anonymized_check_column · retention_event_column) + p2:26-30 (status · activated_at · activated_by)", udtraek: "alle mappings (alle status), også inaktive", kol: [["id", T.uuid], ["entity_type", T.text], ["table_schema", T.text], ["table_name", T.text], ["field_strategies", T.jsonb], ["jsonb_field_strategies", T.jsonb], ["strategy_version", T.int], ["is_active", T.bool], ["created_at", T.tstz], ["updated_at", T.tstz], ["status", T.text], ["internal_rpc_anonymize", T.text], ["internal_rpc_apply", T.text], ["anonymized_check_column", T.text], ["retention_event_column", T.text], ["activated_at", T.tstz], ["activated_by", T.uuid]] },
