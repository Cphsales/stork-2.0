...
## Addendum v3 (blob 66d15100)

Auditeret: `diff-v2-til-v3.patch` (alle 59 linjer — hunks: status-linje · K-1 kilder · K-5 ac 3 · K-5 kilder · dispositions-rækken konfig-lifecycle · princip 5-noten · "Ingen modsigelse"-afsnittet) mod ledger M-1..M-34 (nu inkl. M-31..M-34 og de verbatim Q-R5-referenter). Rammen er ændret af Mathias selv og står i ledgeren: M-34 gør upload til driver-automatik (fremlæggelse → hans ene `krav ok` ligger STADIG efter upload, før godkendelse), M-32/M-33 sanktionerer afledt-med-citat-formen (værn 1 materialitet + 2 ét-skridt: "ja undtagen nummer 3"). Jeg dømmer kun udførelsen.

**(1) Nye ubekræftede afledninger: INGEN.** De eneste nye indholds-påstande er de to sanktionerede afledninger (nedenfor), materialitets-noten i K-5 ac 3 ("auto-hvile udløses først i trin 24" — konsistent med K-5's eksisterende scope-ærlighed, ingen ny påstand) og forfatterens verifikations-deklaration (se pkt. 3).

**(2) De to afledt-med-citat-omskrivninger — begge ÆRLIGE som ét-skridts-afledninger:**
- **K-1 tom-lokation ← M-12/M-14: GODKENDT, tæt.** M-12 ("klienter skal altid kunne kobles på og fra") → "altid" uden forbehold inkluderer den sidste klient → lokation uden klienter kan stå; M-14 beviser uafhængigt at nul-klient-lokationer findes i hans egen model (nedlæggelse kobler alle af, lokationen består). Ét skridt pr. citat, ingen opfundet mellem-præmis — og det er substantielt samme slutning som denne audits runde 1-dom over M-24-læsningen.
- **K-5 straks-virkning ← M-29.5: GODKENDT.** Med Q5-referenten i ledgeren (binært "godkendelse / direkte"; Mathias fik at vide at udkastet krævede godkendelses-livscyklus) er "styres i retigheder" → "rettighed = direkte handling med audit, straks-virkning" ét skridt. Residualen (svaret vælger ikke ordret grenen "direkte") er præcis det bekræftelses-linjen dækker — begge udfald er stadig specificeret (stop → fortrydelses-mekanismen K-6 ac 10).

**(3) v2-betingelserne i v3:** Betingelse 1 er omformet af Mathias selv (M-34): bekræftelses-linjerne blokerer ikke upload, men fremlæggelsen ligger stadig før hans ene `krav ok` — formålet (kravet bliver ikke Mathias-sandt uden hans syn på de to afledninger) er bevaret; linjerne er deklareret i både status-linjen og begge K'er. Betingelse 2 forblev lukket; ledger-hygiejne-flaget fra tillægget er nu også lukket (Q-R5-2/Q-R5-4 leveret verbatim, semantisk matchende M-30.2/M-30.4). Betingelse 3: verifikations-grænse-deklarationen står nu i "Holdt mod låste docs" (alle uden-for-§1.12-citater holdt mod fuld masterplan) — deklareret som krævet; selve efterprøvningen ligger fortsat uden for dette dir (uændret grænse). Re-pin: SKET i headeren (M-1..M-32 @ 54ca276, blob b44c2d26c418).

**Ét ikke-blokerende rest-flag (provenance-hygiejne, ikke forretningsindhold):** fodnote-linjen "Mathias-ord i denne krav-fase: … (M-1..M-30 @ fb8184c)" (v3 linje 424) er IKKE re-pinnet og er nu internt inkonsistent med headeren — og formens sanktion (M-33) samt upload-reglen (M-34) ligger uden for det pinnede interval M-1..M-32. Rettes med én linje ved fold-ind (fodnote-pin → samme blob som header, eller pin M-1..M-34); rører ingen K-påstand.
...
## Plan-audit (blob 423d9b20)

Auditeret: plan-udkast.md (alle 604 linjer) mod krav-doc.md (verificeret byte-identisk med krav-blob 9402164d = v5), recon2.md (672 linjer, begge aktør-blikke læst), mathias-ord.md (M-1..M-40) og ordbog.md. Akse: kun kilde-troskab — teknisk kvalitet er ikke dømt her.

**Efterprøvet og HOLDER (de bærende kilde-referencer):** recon-Code K-2's to-tabel-citat ("cyklus strukturelt umulig — også gyldig opfyldelse") ordret · "grep: 0 hits" for deferrable (recon-Code K-2/V6 ordret) · V11-faktummet ("eneste variant der opfylder ac 2 uden tre-tilstands-semantik") ordret · P01/P02/P04/P05/P06/P07/P08/P12 findes og bærer det planen tilskriver dem (særligt: V4-undtagelsen for gruppe-frakobling er recon-båret — R2-H5: "default afvisning ved nedlagt gælder handling, ikke gruppefrakobling"; V8's "MINUS stadig gældende fravalg" er P05's Bevar-kolonne ordret) · E026/E034/E035/E053/E057/E079/E081/E082/E087/E088/E109/E111-E112/E119-E121/E209 findes og matcher · R2-H1..H8-HALT-tabellen findes (recon2 slutning) — planens R2-H1/R2-H7-referencer er gyldige · recon-Codex' K-6-acceptmatrix findes og planen overtager den loyalt · cnp-kardinalitets-fælden, LENIENT-vinduet, G059, run-db-tests-én-query-begrænsningen: alle recon-bårne som citeret · M-citaterne matcher ledgeren (M-14 ordret m. stavefejl, M-17, M-19, M-21, M-25.1, M-27b, M-28, M-29.5 korrekt mærket som kravets afledning, M-36) · alle kravets 13 plan-fase-afgørelser er truffet INDEN FOR kravets ramme (V1-V13 dækker kravets liste 1:1); kravets defaults er fulgt (V3/V4) eller afvigelsen deklareret (V1: "afviger fra kravets analogi-default" — kravet kalder selv analogien "analogi, ikke krav") · §1-matrixens rækker svarer til kravets ac'er (alle 9 K stikprøvet; K-2 ac 4 og K-4 ac 9 holder sig præcist til kravets egne trin 24-afgrænsninger og slut-effekt-form) · ordbogs-arven: ingen navne-afvigelse uden entry — de 6 [NY ENTRY]-rækker er allerede committet i ordbog.md.

### Fund

**FUND-1 (medium) — K-7 ac 4: kravets "persondata" indsnævres tavst til "direct"** (plan-linje 115-116, 292, 490/S-13). Planen klassificerer `lokationer.adresse` som **indirect persondata** (S-13, "deklareret læsning, recon-flag") og holder samtidig lokations-anonymisering INAKTIV med begrundelsen "ingen **direct**-felter → ingen mapping". Kravets ac 4 siger "ingen **persondata**-felt uden anonymiserings-vej (intet tavst GDPR-hul)" og "Lokations-anonymisering aktiveres først hvis et lokations-felt aktivt klassificeres som **persondata**" — ikke "direct". Efter bogstaven udløser planens egen klassifikation altså aktiverings-klausulen; recon flagede selv punktet (R2-H4: "indirect-PII-behandling … er ikke implicitte"; E107: motoren behandler hverken jsonb eller indirect), men planen disponerer det tavst. Motorens direct-only-adfærd er recon-faktum — indsnævringen af KRAVETS ord er derimod udeklareret. **Forslag:** deklarér læsningen eksplicit i S-13 ("ac 4's aktiverings-klausul læses som direct; indirect uden retention har ingen sletteregel (§11) og dermed ingen anonymiserings-vej at kræve") og lad plan-gaten/Mathias se den — ELLER klassificér adresse `none` med recon-flaget noteret — ELLER levér en vej.

**FUND-2 (mindre) — fejl-attribution: "K-9-fladekontrakt-note"** (plan-linje 308). "læse-RPC'er er underlagt max_rows=1000/30s (K-9-fladekontrakt-note)" — kravet indeholder INTET om max_rows/30s (grep: 0 hits). Kilden er recon-2 (recon-Code K-9-constraints: "max_rows=1000 + 30s timeout begrænser læse-RPC'er" + R2-K9's ord "fladekontrakt"). En læser leder forgæves i kravets K-9. **Forslag:** omskriv til "(recon-2: K-9-fladekontrakt — recon-Code K-9 + R2-K9)".

**FUND-3 (mindre) — fejl-citat: hedge fjernet i anførselstegn** (plan-linje 466/V9). Planen citerer recon-Codex: "cron ikke nødvendig for status ved datoopslag" — recon-Codex skriver "En cron er ikke **nødvendigvis** nødvendig for status ved datoopslag" (R2-K5). Forbeholdet er klippet ud inde i et citat. Substansen bærer stadig planens valg, men citatet skal være ordret eller uden anførselstegn.

**FUND-4 (mindre) — dinglende afgørelses-reference "V-A"** (plan-linje 42 + 215). Matrixen og DDL-kommentaren henviser til "afgørelse V-A" ("se afgørelse") — §3 kender ingen V-A; to-tabel-afgørelsen hedder **S-1**. Byggeren/Codex slår op og finder intet. **Forslag:** ret begge til S-1.

**FUND-5 (mindre) — tælle-fejl: "de fem [NY ENTRY]-ordbogs-rækker"** (plan-linje 604). §4 markerer **seks** rækker [NY ENTRY] (stande · gruppe · status/nedlagt-genåbnelig · gruppens type · kontaktperson · klient). Konsekvensen er allerede neutraliseret — ordbog.md bærer alle 6 committet — men gate-instruksen skal sige seks, ellers kvitterer driveren mod et forkert tal.

### Noter (ikke fund — til gate-panelet)

- "genåbning → gruppens **DA gældende** klienter (M-19)": M-19 bærer "automatisk tilbage"; "da gældende"-præciseringen er V8-modellens konsekvens + recon-Codex E057 — citér "(M-19 + E057/V8)" for præcision.
- "V8-lektionen (bb9ee808)" i Bid 1/K-4-kill er en bilags-lektions-etiket der kolliderer læsemæssigt med planens egen V8-afgørelse — omdøb eller kvalificér ("bilag-V8").
- S-1's fravigelse af §1.12's samme-tabel-form og AUDIT_EXEMPT-udvidelsen for pris-/gruppe-historik er DEKLAREREDE valg med recon-belæg hhv. begrundelses-kommentar (recon: undtagelser er navngivne, ikke generelle — planen navngiver) — substansen er Codex-angrebets/gatens bord, ikke kilde-brud. §6's masterplan-blok lægger fravigelsen synligt på Mathias' bord.
- Verifikations-grænse (uændret): alle `path:linjer @ blob`-referencer til repo-filer samt samtlige "bilag L###"-kill-frø (recon-2-bilaget ligger ikke i dette dir) kan ikke efterprøves her; recon2's evidensregister er deres nærmeste kilde og er konsistent med planens brug i alle stikprøvede tilfælde.
- Plan §0 pinner ledger @ M-1..M-38; M-39/M-40 er workflow-doms uden krav-/plan-substans — ingen konsekvens.

### Konklusion plan-audit: FUND (5)

1 medium (FUND-1 — kravets ac 4-ord indsnævret uden deklaration; recon krævede eksplicit disposition) + 4 mindre (attribution/citat/reference/tælling). Ingen fabrikeret recon-fakta fundet: alle efterprøvede "recon-2 siger"-påstande holdt, ofte ordret. Ingen M-ord-brud. Ingen ordbogs-brud. Planens 13 afgørelser ligger inden for kravets ramme med deklarerede afvigelser. FUND-1 bør disponeres (deklaration eller omklassificering) og FUND-2..5 rettes med fire én-linjes-rettelser før plan-låsning; ingen af dem rører planens struktur.
