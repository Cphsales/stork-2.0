...
- **B1 [stærk, INTERN i master-plan]** — §1.2 angiver retention-enum "time*based / event_based / **legal** / manual", men master-planens egen **Appendix C rettelse 24** siger ordret: "`legal` retention-type fjernes fra systemet. retention_type-enum reduceres til {time_based, event_based, manual, permanent}". §1.13 har den korrekte enum ({time_based, event_based, manual, permanent}). **Bevist sandhed:** rettelse 24 (Appendix C = master-planens egen rettelses-log) + §1.13 vinder; §1.2 blev aldrig opdateret (har stadig "legal", mangler "permanent"). *(Ikke H029 — intern master-plan-staleness.)\_ **[NY]** INGEN rettelse — kun flag.
