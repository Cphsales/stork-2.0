...
3. **Krav skrives:** Mathias + claude-ai-sessionen → hvert **K-n: HVAD** (forretnings-sprog, ingen kode) + **acceptkriterie = slut-effekt INKL. negativer**; bundet til recon-OID **+ citeret mathias-ord-id pr. K** (ledgeren, DEL IV); **hvert recon-fund disponeret** (behandlet/udskudt/ikke-relevant). Sessionen skriver KUN **udkast**: `plan-build/<pakke>/krav-udkast.md` (AI-zone — sandhed-protect slækkes ALDRIG generelt). **Fresh-eyes-audit FØR upload (B1, Mathias 2026-09-03):** en FRISK claude-ai-instans (blind for forfatter-sessionen) auditerer udkastet for **ubekræftede afledninger** (antag-aldrig-audit) → committet audit-fil `plan-build/<pakke>/antag-aldrig-audit.md`; driveren AFVISER upload uden den (mandatet bærer audit-blob-OID — hook-håndhævet). **Audit-fund klassificeres (M-33) FØR de når nogen:** bekræftelse (ét-skridts-afledning af M-ord → batchet i fremlæggelsen) vs ægte spørgsmål (→ spørgsmåls-devil → Mathias). **P-4 (M-35): auditen suppleres af et Codex-KILDETJEK** med komplet versionsbundet kildegrundlag — hver påstand klassificeret {direkte belæg · deklareret afledning · modstrid · mangler kilde} — før upload OG før plan-lås. **ÉT delta-batch pr. runde (M-39 pkt. 2):** kildetjek (P-4) og buildability kører PARALLELT på SAMME blob; ALLE fund foldes ind i ÉN ny version → ét audit-addendum → én upload → én re-bind — og der bedes ALDRIG om `krav ok` mens et tjek stadig er åbent (M-37-situationen må ikke gentages).
...
| 25 | Divergens-klassificering: påstand→Mathias, ordlyd→AI-spor, uklassificeret=påstand | (dette commit) · consolidate-recon.mjs + 2.C |
