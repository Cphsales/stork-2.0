...
- **Hærdet-register (Mathias 2026-09-03 — u-hærdet gate-transport må aldrig gå i drift; `haerdet-register.json` + selftest):** HVERT modul gate-dommen afhænger af (transitiv import fra gate-entrypoints + producenter af gatede artefakter) SKAL bære en register-entry med Codex-pas-commit + **blob-OID af den pas'ede version** — et modul ændret efter sit pas ⇒ blob-mismatch ⇒ **CI rød** indtil nyt pas + register-opdatering. Byg af build-/chain-gate-transport (Fase 4/5) tilføjer sine moduler FØR deres dom tæller.
...
3. **build-proof** (2.C): bijektion + effect-harness + afvisnings-sti-coverage + config-mutant-kill + claim_graph + async_reviews + prover grøn. **P-7 (M-35):** Codex leverer pr. bid et EGET resultat om FAKTISK diff ⊨ krav/plan (manglende kravdel · ekstra beslutning · test-tilpasning), bundet til aktør + base_oid + præcis kode-OID i review-kontrakten (async_reviews kræver dermed et BESTEMT reviewer-sæt, ikke blot PASS pr. bid).
