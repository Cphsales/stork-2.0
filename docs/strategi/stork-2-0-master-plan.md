...
Reflekterer T9-omstart-rammen fra 2026-05-17 (mathias-afgoerelser, 15 punkter). Rettelse 35 (2026-05-18) erstatter pre-omsadlings-tekst der modsagde rammen.
...
**Permission-model (T9-omstart-rammen 2026-05-17, punkt 1-4):**
...
- **To akser pr. (rolle × område × page × tab):**
  - **Kan tilgå** (kan_se/tilgå + kan_skrive)
  - **Synlighed** på data: `Sig selv` / `Hiraki` / `Alt` (kun tre værdier)
- Begge akser sættes UI-styret. Samme rolle kan have forskellig synlighed på forskellige elementer (eksempel: TM-sælger har `Sig selv` på vagtplan-page og `Hiraki` på kalender-page)
...
**ÉT træ-anker (T9-omstart-rammen punkt 2, 5, 6, 7, 8, 9):**

- Organisations-træet er ÉT træ: Copenhagen Sales → afdelinger → teams → medarbejdere
- **`Hiraki` udledes af medarbejderens placering i organisations-træet.** Ser egen knude og alt under. Knude-løs medarbejder + synlighed=`Hiraki` = ser intet
- **Klienter tilknyttes kun knuder af type team.** Aldrig afdelings-knuder
- **Knude-løs medarbejder er gyldig tilstand.** Når et team lukkes, bliver medarbejdere knude-løse; de forbliver ansatte og kan tildeles ny placering i UI
- **Ingen stabs-team i 2.0.** Stabs-konceptet fra 1.0 udgår fuldstændig
- **Cross-team-adgang løses via rolle med synlighed=`Hiraki` eller `Alt`** — ikke ved at give medarbejdere flere placeringer

**Superadmin (T9-omstart-rammen punkt 10 + vision-princip 2):**
...
- BEFORE UPDATE/DELETE-trigger på `employees.role_id` og på superadmin-permission-rækker
- Konfig-tabel definerer minimum N (default: 2) medarbejdere der bevarer superadmin
- Hvis mutation ville reducere superadmin-antal under N → RAISE med klar besked
- Sletning af medarbejder med superadmin-rolle udløser samme tjek
- N er UI-redigerbart i konfig-tabel

**Struktur-styring (T9-omstart-rammen punkt 11, 12):**

- Alle navne på afdelinger og teams oprettes i UI. Krav-dokumenter specificerer ingen konkrete navne (undtagelse: `Copenhagen Sales` root + `Ejere` afdeling som bootstrap)
- Hvem der må oprette/ændre/lukke knuder styres via rettigheder i UI på relevante permission-elementer. Ingen særlig ledelses-handling-kategori; struktur-adgang er almindelig rettighed

**Fortrydelses-mekanisme (T9-omstart-rammen punkt 13, 14):**

- Alle ændringer med gældende dato følger fortrydelses-mekanisme: gældende dato → godkendelse → fortrydelses-periode → ændring kan rulles tilbage i UI indtil periodens udløb → derefter permanent
- Gælder struktur-ændringer, medarbejder-placeringer, klient-flytninger
- Fortrydelses-periodens længde konfigureres i UI pr. change_type. Ingen hardkodet værdi
- Central tabel `core_identity.pending_changes` + konfig-tabel `core_identity.undo_settings`
- Central apply-gate `pending_change_apply(change_id)` verificerer due-check (status='approved' AND undo_deadline <= now() AND effective_from <= current_date)
- Cron `pending_changes_apply_due` (hver minut) er selection-filter; apply-gate er sandheds-grænse

**Org-træ:**

- `core_identity.org_nodes` er identity-only (id, created_at, updated_at) — stable identitet på tværs af versions
- `core_identity.org_node_versions` er primær mutable lagring med `effective_from` / `effective_to`. Pr. node_id én open-ended version (effective_to IS NULL); historiske versions har effective_to sat
- `parent_id` på versions refererer `org_nodes.id`. Selv-refererende træ
- Cycle-detection-trigger via plpgsql BEFORE INSERT/UPDATE
- Team-knuder kan ikke have børn (krav-dok 3.1) — verificeres af trigger
- Vilkårligt antal niveauer. Mellem-niveauer tilføjes som data

**Closure-table for subtree-evaluering** (`core_identity.org_node_closure`):

- Materialiseret hjælpe-tabel der erstatter rekursiv CTE i policy-prædikater
- Current-state-derived fra `org_node_versions` effective at current_date. ALDRIG future-dated
...
- Vedligeholdes af AFTER INSERT/UPDATE/DELETE-trigger på `org_node_versions`
- Helper `acl_subtree_org_nodes(employee_id)` returnerer descendant-array via indexed lookup
- Helper `acl_subtree_employees(employee_id)` returnerer employee-IDs i caller's subtree (placement på node i acl_subtree_org_nodes)
- Begge helpers markeret STABLE, SECURITY INVOKER, deterministisk search_path
...
**Medarbejder-knude-placering:**

- Versioneret med `effective_from` + `effective_to` i `core_identity.employee_node_placements`
- Partial unique på `(employee_id) WHERE effective_to IS NULL` — én aktiv placering ad gangen
- Exclusion-constraint mod overlappende perioder (`daterange(effective_from, coalesce(effective_to, 'infinity'), '[)')`)
- Skift-handler lukker gammel placement (sætter effective_to) og åbner ny i samme transaktion
...
**Helper-funktioner pr. synlighed:**

- `Sig selv` — employee-match (`acl_self(target_employee_id) = caller`)
- `Hiraki` — closure-table-lookup via `acl_subtree_org_nodes` / `acl_subtree_employees`
- `Alt` — alle rækker (`acl_all() = true`)

**Permission-resolution:**

- `permission_resolve(role_id, element_type, element_id)` returnerer (can_access, can_write, visibility)
- Arve-aware lookup: tab → page → area → default-deny
- Brug i RPC-bodies for permission-evaluering: `has_permission(page_key, tab_key, can_edit)`-helper er fallback-konsument af resolve

**Write-mekanik (master-plan §1.1's pattern):**
...
19. **FK-coverage** pr. tabel — fitness-script identificerer kolonner med suffiks `_id` og verificerer at de har FK constraint mod kolonnens ankerentitet. Tilladte undtagelser dokumenteres i `scripts/fitness.mjs` allowlist (`FK_COVERAGE_EXEMPTIONS`) med begrundelse (fx eksterne reference-ID'er som `external_id`, `client_crm_match_id`)
20. **DB-test tx-wrap pr. immutable INSERT** — fitness-script (`db-test-tx-wrap-on-immutable-insert`) scanner `supabase/tests/**/*.sql` og kræver `begin;` + `rollback;` fil-niveau-wrap for tests der INSERT'er i tabeller på liste over strict immutability + conditional immutability + lifecycle-DELETE-restricted (`TX_WRAP_REQUIRED_FOR_TEST_INSERT`). Allowlist-kommentar `-- no-transaction-needed: <reason>` undertrykker check pr. fil. Falsk-negativ-afgrænsning: fanger ikke RPC-side-effects der INSERT'er indirekte (kandidat til senere Mønster D-udvidelse).
...
- **Sammenkobling eksplicit i datamodellen.** Relations er data, ikke implicit i kode. FK-constraints er obligatoriske mellem relaterede entiteter. Håndhævet via CI-blocker 19 (FK-coverage).

- **Konfiguration har livscyklus.** Tabeller med data-håndtering-konfiguration (anonymization_strategies, anonymization_mappings, break_glass_operation_types) gennemgår draft → tested → approved → active. Aktivering er bevidst UI-handling, ikke automatik. Bootstrap-strategier seedet som `approved`; aktivering af `gdpr_responsible` via UI som pre-cutover-step.
...
| Stack                 | React + TypeScript + Supabase. Frontend hosting via managed-service (åben, se Appendix B); selv-hosting afvist |
...
| Frontend hosting-platform   | Vercel vs. Cloudflare Pages. Begge er managed-service med preview-deploys per PR. Vercel: bedst React DX, dyrere ved bandwidth. Cloudflare: billigere bandwidth, mindre moden DX. Afgøres ved tilkobling i samme pakke som første frontend-side. Selv-hosting er afvist (Appendix A) |
...
| 32  | Frontend hosting-ramme låst: managed-service-model bevares (matcher Lovable-hostingen 1.0 bruger). Selv-hosting eksplicit afvist pga. infrastruktur-vedligehold ikke har plads i to-personers projekt. **Specifik platform (Vercel vs. Cloudflare Pages) er åben** — afgøres ved tilkobling i samme pakke som første frontend-side, så Microsoft Entra-redirect-URLs og preview-deploys etableres i kontekst af reelt arbejde. Se Appendix B for valg-kriterier.                                                                                                                                                                                                                                                                                                                                                                                                 |
