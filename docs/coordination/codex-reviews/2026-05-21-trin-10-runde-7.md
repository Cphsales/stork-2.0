...
**Reasoning:** xhigh
**Max ord:** 350
**Command:** `scripts/codex-review.sh docs/coordination/trin-10-plan.md 7 --xhigh --phase=plan` (re-run via samme args inkl. flags)
...
session id: 019e479e-30c7-7d83-9843-dda7a11d96ce
--------
user
Læs disse filer:
...
Max 350 ord output. Brug marker-format fra niveau 1-prefix (KRITISK/MELLEM/LAV/HUL/OPTIMERING-FORSLAG/G-NUMMER-KANDIDAT etc.). Hvis du ikke har fund: skriv "APPROVAL — Runde 7".
codex
Jeg læser de to angivne dokumenter først, så svaret kan følge den lokale marker-protokol og den konkrete planfase for trin-10.
exec
...
codex
Jeg har formålet og prefixen nu. Næste skridt er at læse de tekniske T10-steps og krydstjekke de steder, V7 selv siger er ændret: aktiv-check, grants/RLS/session-vars, tests og fitness-script.
exec
...
exec
/bin/bash -lc "sed -n '620,1060p' docs/coordination/trin-10-plan.md" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc 'rg -n "''^### T10'"\\.|"'^## Fundament|''^## Konklusion|T10'"\\.7b|t10_client_active_check|LEGACY_IS_ACTIVE_EXEMPT|client_node_place|_apply_client_place|grant .*authenticated|set_config|client_field_definitions_list\" docs/coordination/trin-10-plan.md" in /home/mathias/stork-2.0
 succeeded in 0ms:
17:| 1   | KRITISK  | T10.7 (FK)         | FK sikrer KUN eksistens, ikke at klient er aktiv. Krav-dok §2.5.2: "Inaktiv klient bliver stående for historik, men kan ikke vælges som ny team-tilknytning." T9-wrapper `client_node_place` (`20260518000007:140-170`) validerer permission + team-only, men ikke aktiv klient. T9-supplement `_apply_client_place` (`20260520000000:285-352`) validerer team-only + team-aktiv, men ikke klient-aktiv. Krav-dok §3.4 siger "valideres at klienten faktisk findes" — sammen med §2.5.2 betyder det: findes + aktiv. Plus: pending kan oprettes mens klient aktiv og applies efter deaktivering → apply-pathen SKAL også tjekke. | **ACCEPT.** Ny step T10.7b: CREATE OR REPLACE begge RPC'er med aktiv-check **og superadmin-bypass** (Mathias 2026-05-21: "superadmin må alt"). Wrapper-rækkefølge: has_permission → team-check → klient-eksistens (P0002) → klient-aktiv (22023 hvis ikke superadmin). Apply-handler: tilføj klient-eksistens (P0002) + klient-aktiv (P0001 hvis ikke superadmin) FØR INSERT/UPDATE. `client_node_close` rør IKKE. | T10.7b (ny) + T10.15          |
19:| 3   | MELLEM   | T10.15             | Smoke-test dækker FK-eksistens + ON DELETE RESTRICT, men ikke det vigtigste forretningskrav (krav-dok §2.5.2: inaktiv klient kan ikke vælges).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | **ACCEPT.** Ny separat smoke-test `t10_client_active_check.sql` med 4 scenarier: aktiv place success, inaktiv place rejection (22023), pending-mens-aktiv + deaktiver + apply rejection (P0001), close virker på inaktiv klient.                                                                                                                                                                                   | T10.15 + ny test-fil          |
22:**Rettigheds-grænse (Mathias-bekræftet 2026-05-21):** Trin 10 introducerer ingen nye permission-koncepter ift. T9. Permission-modellen (`has_permission` resolver + areas/pages/tabs/grants) er etableret i T9; trin 10 udvider den med 2 nye pages under `org_structure`-area. Ingen klient-baseret adgangs-scope (det ville være senere pakke hvis besluttet). Aktiv-check (T10.7b) er en forretnings-**invariant**, ikke en permission-check — den håndhæves uafhængigt af caller-identitet.
36:| 3   | KRITISK/FUNKTIONELT | T10.1 + T10.2 + T10.8 + T10.9 + T10.11 + T10.12 | `has_permission(p_page, NULL, false)` med `p_tab_key=NULL` springer tab-resolver over (`20260518000010_t9_seed_owners.sql:35`) og prøver kun page/area-grants. T10.13 seeder kun TAB-grants → read-paths matcher INGEN grant og returnerer false → SELECT-policy + read-RPC'er tilbageholder data for legitime brugere med kun `clients/manage`-grant. | Mathias-terminal | **ACCEPT.** Skift alle read-paths til tab-aware: `has_permission('clients', 'manage', false)` og `has_permission('client_field_definitions', 'manage', false)`. Berører SELECT-policies (T10.1 + T10.2), client_get/client_list/client_field_definitions_list (T10.12), client_logo_get (T10.11). Write-paths bruger allerede 'manage' tab — konsistent.                                               |
37:| 4   | KRITISK-FABRIKATION | T10.16                                          | Plan refererer `FK_COVERAGE_EXEMPTIONS`-allowlist i scripts/fitness.mjs, men den findes IKKE. Master-plan §3 punkt 19 specificerer fitness-checken men den er ikke implementeret. T9-migration-kommentar (`20260518000004_t9_client_node_placements.sql:5`) er forhåndsdokumentation, ikke nuværende fitness-check.                                    | Code-validering  | **ACCEPT.** Omformulér T10.16: i stedet for "fjern client_id fra allowlist", verificér at FK-coverage-checken er implementeret; hvis ikke, ingen fitness-script-ændring nødvendig for FK. **Plus** tilføj `core_identity.client_field_definitions_list` til `LEGACY_IS_ACTIVE_EXEMPT_FUNCTIONS` (fund #5). G-nummer-kandidat for "FK-coverage-fitness-check ikke implementeret per master-plan §3.19". |
38:| 5   | KRITISK             | T10.12 client_field_definitions_list            | RPC bruger `where p_include_inactive or is_active = true` — matcher fitness-check `legacy-is-active-readers` (`scripts/fitness.mjs:830-892`) regex. client_field_definitions har KUN is_active (ingen status-kolonne), så funktionen skal allowlist'es i `LEGACY_IS_ACTIVE_EXEMPT_FUNCTIONS`.                                                          | Code-validering  | **ACCEPT.** Tilføj `core_identity.client_field_definitions_list` til allowlisten via T10.16's fitness-script-ændring.                                                                                                                                                                                                                                                                                  |
39:| 6   | KRITISK             | T10.15 `t10_client_node_placements_fk.sql`      | Smoke-test INSERT'er i `core_identity.client_node_placements` som er på `TX_WRAP_REQUIRED_FOR_TEST_INSERT` (`scripts/fitness.mjs:110`). Fitness-check `db-test-tx-wrap-on-immutable-insert` (`:901-924`) kræver `begin;` + `rollback;` på linje-niveau.                                                                                                | Code-validering  | **ACCEPT.** Eksplicit `begin;` + `rollback;` wrap-pattern i T10.15's FK-test specifikation. T10.7a's fixture-INSERT i T9-tests sker indenfor eksisterende BEGIN/ROLLBACK (verificeret: `t9_placements.sql:9` + `:213`, `t9_backdated_historical_traversal.sql:9` + `:311`).                                                                                                                            |
51:| 1   | KRITISK  | T10.1 + T10.2 | Tabellerne har kun `GRANT SELECT to authenticated`. Mangler `GRANT INSERT, UPDATE` der er nødvendigt før RLS-policy/session-var-vejen kan virke for write-RPC'erne (T10.8-T10.11). Bryder niveau 1-prefixens GRANT + policy + session-var-tre-pak. | **ACCEPT.** Tilføj `grant insert, update on table core_identity.clients to authenticated` i T10.1 og tilsvarende for `client_field_definitions` i T10.2. Ingen DELETE-grant (inaktivering via is_active, ikke DELETE). Matcher T1's mønster for `core_compliance.data_field_definitions`. | T10.1 + T10.2 |
61:| 1   | KRITISK  | T10.13  | T10.13 sætter `stork.source_type` + `stork.change_reason` men IKKE `stork.t9_write_authorized = 'true'` som T9-supplements INSERT-policies på `permission_pages` / `permission_tabs` / `role_permission_grants` kræver. Brudt niveau 1-prefix end-to-end-tjek for INSERT/UPDATE/DELETE-veje (GRANT + policy + session-var-tre-pak). | **ACCEPT.** Tilføj `select set_config('stork.t9_write_authorized', 'true', false);` før INSERTs i T10.13. Opdater Fundament-tjek-tabel. | T10.13 + Fundament-tjek |
110:| `core_compliance.stork_audit()` trigger-funktion                                                       | Refereret af alle T9-tabeller (`20260518000004_t9_client_node_placements.sql:55` etc.)                         | Generel audit-trigger der bruges `AFTER INSERT OR UPDATE OR DELETE` på alle write-tabeller. Skriver til `core_compliance.audit_log` via `audit_filter_values`.                                                                                                                                                                                                                                          |
112:| `core_identity.client_node_placements` (tabel)                                                         | `supabase/migrations/20260518000004_t9_client_node_placements.sql:13-24`                                       | `id, client_id uuid NOT NULL (UDEN FK — Plan V6 Valg 4: FK tilføjes i trin 10), node_id FK → org_nodes, effective_from/to date, applied_at, created_by_pending_change_id`.                                                                                                                                                                                                                              |
113:| T9-supplement `client_node_placements_select` policy                                                   | `supabase/migrations/20260520000000_t9_supplement.sql:665-683`                                                 | `using (is_admin() OR node_id = ANY(acl_subtree_org_nodes_at(current_employee_id(), coalesce(current_setting('stork.t9_read_at_date',true)::date, current_date))))`. **Trin 10 ændrer ikke policy** — kun tilføjer FK.                                                                                                                                                                                  |
114:| `core_identity._apply_client_place(p_payload jsonb, p_pending_change_id uuid)`                         | `supabase/migrations/20260518000004_t9_client_node_placements.sql:86-119`                                      | SECURITY DEFINER apply-handler. INSERT i client_node_placements.                                                                                                                                                                                                                                                                                                                                        |
115:| `core_identity._apply_client_close(p_payload jsonb, p_pending_change_id uuid)`                         | `supabase/migrations/20260518000004_t9_client_node_placements.sql:123-147`                                     | SECURITY DEFINER apply-handler. UPDATE effective_to.                                                                                                                                                                                                                                                                                                                                                    |
116:| `core_identity.client_node_place(p_client_id, p_node_id, p_effective_from)` wrapper                    | `supabase/migrations/20260518000007_t9_public_wrapper_rpcs.sql:140-170`                                        | Wrapper-RPC med `has_permission('client_placements', 'manage', true)`. Pre-checker node_id = team. Opretter pending_change.                                                                                                                                                                                                                                                                             |
125:| T9-smoke-test `t9_placements.sql` BLOCK T5/T6/T7                                                       | `supabase/tests/smoke/t9_placements.sql:137-200`                                                               | Bruger client_id-værdier i `_apply_client_place`-payload uden tilsvarende clients-fixture. FK-aktivering brækker T5/T7.                                                                                                                                                                                                                                                                                 |
126:| T9-smoke-test `t9_backdated_historical_traversal.sql` BLOCK 3                                          | `supabase/tests/smoke/t9_backdated_historical_traversal.sql:165-305`                                           | Linje 167, 172, 185, 276 INSERT'er direkte i client_node_placements + kalder `_apply_client_place` med tilfældige client_id'er. Brækker ved FK.                                                                                                                                                                                                                                                         |
128:| T9-mønster for ny tabel i core_identity                                                                | `supabase/migrations/20260518000001_t9_org_nodes.sql:25-41`                                                    | `create table` + `enable rls` + `force rls` + `revoke all from public/anon/service_role` + `grant select to authenticated` + `create policy *_select` + `create trigger *_audit AFTER INSERT/UPDATE/DELETE EXECUTE FUNCTION core_compliance.stork_audit()`.                                                                                                                                             |
129:| T9 RPC-mønster                                                                                         | `supabase/migrations/20260518000007_t9_public_wrapper_rpcs.sql:9-44`                                           | SECURITY DEFINER + `set search_path = ''` + `if not has_permission(...) then raise 42501` + `set_config('stork.source_type'/'change_reason')` + body + `revoke execute from public, anon` + `grant execute to authenticated`.                                                                                                                                                                           |
130:| `LEGACY_IS_ACTIVE_EXEMPT_FUNCTIONS` i fitness-script                                                   | `scripts/fitness.mjs:149-154`                                                                                  | Allowlist for funktioner der bruger `is_active = true` filter uden tilsvarende `status = 'active'`-check. Trin 10 tilføjer `core_identity.client_field_definitions_list` (T10.16) fordi `client_field_definitions` har kun is_active, ingen status-kolonne. **FK_COVERAGE_EXEMPTIONS findes IKKE** — master-plan §3.19 ikke implementeret (V6-fund #4).                                                 |
153:- FK `core_identity.client_node_placements.client_id` → `core_identity.clients.id`
155:- SECURITY DEFINER RPC'er: client_upsert (uden logo), client_set_active, client_field_definition_upsert (uden p_match_role), client_logo_set, client_logo_clear, client_logo_get, client_get, client_list, client_field_definitions_list
159:- Fitness-script-opdatering (tilføj `client_field_definitions_list` til `LEGACY_IS_ACTIVE_EXEMPT_FUNCTIONS`)
178:**Begrundelse:** Master-plan §1.11 placerer klienter i core_identity. T9's `client_node_placements` ligger allerede i core_identity. Konsistent skema-placering.
230:  - **Plan-konsekvens:** T10.7b CREATE OR REPLACE både `client_node_place` (wrapper) og `_apply_client_place` (apply-handler) med aktiv-check og `is_admin()`-bypass. `client_node_close` rør IKKE — lukning er legitim ved deaktivering. Eksistens-check (P0002) bypasses IKKE for superadmin (FK håndhæver alligevel).
238:### T10.1 — CREATE TABLE `core_identity.clients`
284:  grant select on table core_identity.clients to authenticated;
288:  grant insert, update on table core_identity.clients to authenticated;
314:### T10.2 — CREATE TABLE `core_identity.client_field_definitions`
359:  grant select on table core_identity.client_field_definitions to authenticated;
361:  grant insert, update on table core_identity.client_field_definitions to authenticated;
384:### T10.3 — Udvid `core_compliance.is_permanent_allowed`-allowlist
434:### T10.4 — Klassifikation i `core_compliance.data_field_definitions`
441:  select set_config('stork.source_type', 'migration', false);
442:  select set_config('stork.change_reason',
444:  select set_config('stork.allow_data_field_definitions_write', 'true', false);
499:### T10.5 — Omskrive `core_compliance.audit_filter_values` med clients-fields-jsonb-walking
621:### T10.6 — `clients_validate_fields`-trigger (LENIENT default)
679:### T10.7 — FK fra `client_node_placements.client_id` til `clients.id`
686:  alter table core_identity.client_node_placements
687:    add constraint client_node_placements_client_id_fkey
693:- **Migration-fil:** `supabase/migrations/<ts>_t10_client_node_placements_fk.sql`
696:### T10.7a — Opdater T9-smoke-tests med clients-fixture
699:- **Hvad:** Tre T9-smoke-tests INSERT'er `client_id`-værdier uden clients-fixture. Når T10.7 aktiveres, brækker testene. T10.7a seeder en `core_identity.clients`-fixture FØR hver berørt INSERT/\_apply_client_place-kald.
703:  - FØR T5's `_apply_client_place`-kald (linje 138): seed clients-fixture med `v_client_id`.
711:  Pattern (én blok per test, FØR første client_node_placements-write):
715:  perform set_config('stork.source_type', 'manual', true);
716:  perform set_config('stork.change_reason', 'test fixture for client_node_placements', true);
717:  perform set_config('stork.allow_clients_write', 'true', true);
726:### T10.7b — CREATE OR REPLACE `client_node_place` + `_apply_client_place` med klient-aktiv-check (V7 — Mathias-terminal-V6 #1)
739:  create or replace function core_identity.client_node_place(
783:  revoke execute on function core_identity.client_node_place(uuid, uuid, date) from public, anon;
786:  create or replace function core_identity._apply_client_place(
834:    from core_identity.client_node_placements
841:      insert into core_identity.client_node_placements
844:        (select min(effective_from) from core_identity.client_node_placements
848:      update core_identity.client_node_placements
854:      update core_identity.client_node_placements
857:      insert into core_identity.client_node_placements
864:  revoke execute on function core_identity._apply_client_place(jsonb, uuid) from public, anon, authenticated;
869:  - `t9_placements.sql` T6 (`_apply_client_place` på department + `gen_random_uuid`-klient): team-check fejler først → uændret semantik.
873:- **Migration-fil:** `supabase/migrations/<ts>_t10_client_active_check.sql`
876:### T10.8 — `client_upsert` RPC (uden logo-parametre)
905:    perform set_config('stork.source_type', 'manual', true);
906:    perform set_config('stork.change_reason', p_change_reason, true);
907:    perform set_config('stork.allow_clients_write', 'true', true);
931:  grant execute on function core_identity.client_upsert(text, jsonb, text, boolean, uuid) to authenticated;
938:### T10.9 — `client_set_active` RPC
960:    perform set_config('stork.source_type', 'manual', true);
961:    perform set_config('stork.change_reason', p_change_reason, true);
962:    perform set_config('stork.allow_clients_write', 'true', true);
974:  grant execute on function core_identity.client_set_active(uuid, boolean, text) to authenticated;
981:### T10.10 — `client_field_definition_upsert` RPC (uden p_match_role; V3: immutable key + pii_level-downgrade-block)
1016:    perform set_config('stork.source_type', 'manual', true);
1017:    perform set_config('stork.change_reason', p_change_reason, true);
1018:    perform set_config('stork.allow_client_field_definitions_write', 'true', true);
1062:  grant execute on function core_identity.client_field_definition_upsert(text, text, text, text, text, boolean, integer, boolean, uuid) to authenticated;
1071:### T10.11 — Logo-RPC'er (`client_logo_set` + `client_logo_clear` + `client_logo_get`)
1098:    perform set_config('stork.source_type', 'manual', true);
1099:    perform set_config('stork.change_reason', p_change_reason, true);
1100:    perform set_config('stork.allow_clients_write', 'true', true);
1127:    perform set_config('stork.source_type', 'manual', true);
1128:    perform set_config('stork.change_reason', p_change_reason, true);
1129:    perform set_config('stork.allow_clients_write', 'true', true);
1159:  grant execute on function core_identity.client_logo_set(uuid, bytea, text, text, text) to authenticated;
1160:  grant execute on function core_identity.client_logo_clear(uuid, text) to authenticated;
1161:  grant execute on function core_identity.client_logo_get(uuid) to authenticated;
1168:### T10.12 — Read-RPC'er (`client_get`, `client_list`, `client_field_definitions_list`)
1218:  create or replace function core_identity.client_field_definitions_list(p_include_inactive boolean default false)
1224:      raise exception 'client_field_definitions_list: permission_denied' using errcode = '42501';
1234:  revoke all on function core_identity.client_field_definitions_list(boolean) from public, anon;
1235:  grant execute on function core_identity.client_get(uuid) to authenticated;
1236:  grant execute on function core_identity.client_list() to authenticated;
1237:  grant execute on function core_identity.client_field_definitions_list(boolean) to authenticated;
1244:### T10.13 — Seed permissions i grant-modellen
1251:  select set_config('stork.source_type', 'migration', false);
1252:  select set_config('stork.change_reason',
1256:  select set_config('stork.t9_write_authorized', 'true', false);
1293:### T10.14 — Master-plan-rettelser (jf. krav-dok §7)
1299:  - §4 trin 10-række: erstat hele cellen med: "Klient-skabelon (core_identity.clients + client_field_definitions + logo + is_active + FK fra client_node_placements + has_permission-RPCs)". Migration-tekst og crm_match_id-tekst fjernes.
1305:### T10.15 — Smoke-tests (6 stk i V7)
1308:- **Hvad:** Seks smoke-tests dækker centrale flows. V7 tilføjer `t10_client_active_check.sql` for T10.7b-leverancen.
1314:  | `supabase/tests/smoke/t10_client_field_definitions.sql`  | client_field_definition_upsert (INSERT + UPDATE), is_active toggle, client_field_definitions_list respekterer p_include_inactive. **Audit-PII-hashing:** insert med pii_level='direct' key i fields → audit_log har sha256-hash. **V3 (Codex V2 KRITISK-SIKKERHEDSHUL):** UPDATE af `key` afvises (errcode 22023). UPDATE af pii_level direct → none afvises (errcode 22023). pii_level none → indirect → direct accepteres.                                                                                                                                                                                                                                                                                                                                     |
1316:  | `supabase/tests/smoke/t10_client_node_placements_fk.sql` | FK virker: INSERT med ikke-eksisterende client_id fejler. DELETE af klient med åbne placements fejler RESTRICT. **V6 (Code-validering fund #6):** Test SKAL være `begin;` + `rollback;`-wrapped (linje-niveau) — `core_identity.client_node_placements` er på `TX_WRAP_REQUIRED_FOR_TEST_INSERT` (`scripts/fitness.mjs:110`). Fitness-check `db-test-tx-wrap-on-immutable-insert` blokerer ellers.                                                                                                                                                                                                                                                                                                                                                               |
1318:  | `supabase/tests/smoke/t10_client_active_check.sql` (V7)  | T1: opret aktiv klient → `client_node_place` succeeds → pending oprettes → approve+apply succeeds → placement findes. T2: `client_set_active(client_id, false)` → ny `client_node_place` på samme klient + nyt team → forvent **22023 `client_inactive`**. T3 (apply-path-scenarie Mathias #1): opret pending mens klient aktiv → deaktiver klient → approve+apply → forvent **P0001 `apply_client_place: client_inactive`**. T4: `client_node_close` på inaktiv klient → success (ingen aktiv-check her). **T5 superadmin-bypass (Mathias 2026-05-21):** med superadmin-auth, place på inaktiv klient → success (bypass på aktiv-check, ikke på eksistens-check). Test SKAL være `begin;` + `rollback;`-wrapped (`client_node_placements` på TX_WRAP_REQUIRED). |
1320:- **Afhængigheder:** alle migrations i T10.1-T10.13 + T10.7b for active_check-test
1324:### T10.16 — Fitness-script-opdatering (V6 omformuleret)
1328:  1. **FK-coverage-allowlist:** `FK_COVERAGE_EXEMPTIONS`-allowlist findes IKKE i nuværende `scripts/fitness.mjs` — master-plan §3 punkt 19 er ikke implementeret endnu. Hvis check tilføjes senere, vil `client_node_placements.client_id` ikke længere være en exemption-kandidat (FK eksisterer efter T10.7). **Ingen fitness-script-ændring nødvendig for FK i V6.**
1329:  2. **R7d-allowlist (KRÆVET):** Tilføj `core_identity.client_field_definitions_list` til `LEGACY_IS_ACTIVE_EXEMPT_FUNCTIONS` (`scripts/fitness.mjs:149-154`). RPC'en bruger `where p_include_inactive or is_active = true` for at filtrere aktive felt-definitioner. `client_field_definitions` har KUN is_active (ingen status-kolonne) — matcher allowliste-kommentaren: "T9-tabellerne har is_active som lifecycle-signal alene; ingen status-kolonne. Disse er allowlist'et nedenfor."
1333:  // I LEGACY_IS_ACTIVE_EXEMPT_FUNCTIONS-Set (~linje 149-154):
1334:  const LEGACY_IS_ACTIVE_EXEMPT_FUNCTIONS = new Set([
1336:    "core_identity._apply_client_place",
1337:    "core_identity.client_node_place",
1339:    "core_identity.client_field_definitions_list", // V6 — T10.12 RPC; client_field_definitions har kun is_active, ingen status
1344:- **Afhængigheder:** T10.12 (`client_field_definitions_list`-RPC skal eksistere så allowlist-entry refererer reel funktion)
1349:## Fundament-tjek-passeret
1354:| Hver SELECT-policy bred nok til legitime læsere                | ja     | T10.1, T10.2 — has_permission('clients'/'client_field_definitions', 'manage', false) **tab-aware (V6-fix)**. T10.13 seeder kun tab-grants → null-tab matcher ikke; 'manage' matcher. T9-supplement's ACL-scoped policy på client_node_placements bevares uændret.                                              |
1367:| `t10_client_node_placements_fk.sql` | FK afviser ikke-eksisterende client_id; ON DELETE RESTRICT                                                            | grøn             |
1369:| `t10_client_active_check.sql` (V7)  | Aktiv place success + inaktiv place 22023 + pending+deaktiver+apply P0001 + close inaktiv success + superadmin-bypass | grøn             |
1393:  - T9-supplement's `client_node_placements_select` policy uændret
1474:## Konklusion
1482:- **Ny T10.7b:** CREATE OR REPLACE `client_node_place` + `_apply_client_place` med klient-aktiv-check og **superadmin-bypass**. Krav-dok §2.5.2 håndhæves i både wrapper og apply-path (apply fanger pending oprettet mens aktiv, applied efter deaktivering).
1483:- T10.15: ny smoke-test `t10_client_active_check.sql` (5 scenarier inkl. superadmin-bypass).
1492:- T10.15 `t10_client_node_placements_fk.sql`: eksplicit `begin;` + `rollback;` wrap — `client_node_placements` på `TX_WRAP_REQUIRED_FOR_TEST_INSERT` (Code-validering #6).
1493:- T10.16 omformuleret: `FK_COVERAGE_EXEMPTIONS` findes ikke i nuværende fitness-script (master-plan §3.19 ikke implementeret); plan tilføjer i stedet `core_identity.client_field_definitions_list` til `LEGACY_IS_ACTIVE_EXEMPT_FUNCTIONS` (Code-validering #4 + #5). G-nummer for FK-coverage-check.
1497:- T10.1 + T10.2: tilføjet `grant insert, update on table ... to authenticated` — RLS-policy + session-var kan ikke virke før DML-GRANT er på plads. Ingen DELETE-grant (inaktivering via is_active).
1501:- T10.13: tilføjet `set_config('stork.t9_write_authorized', 'true', false)` før INSERTs — krævet af T9-supplement's INSERT-policies på permission_pages/tabs/role_permission_grants (Codex V3 KRITISK).

exec
/bin/bash -lc "sed -n '260,380p' supabase/migrations/20260520000000_t9_supplement.sql" in /home/mathias/stork-2.0
exec
/bin/bash -lc "sed -n '1,230p' supabase/migrations/20260518000007_t9_public_wrapper_rpcs.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
    and effective_from <= v_effective_from
    and (effective_to is null or effective_to > v_effective_from)
  limit 1;
...
exec
/bin/bash -lc "sed -n '1060,1525p' docs/coordination/trin-10-plan.md" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc "sed -n '1,240p' scripts/fitness.mjs" in /home/mathias/stork-2.0
 succeeded in 0ms:
#!/usr/bin/env node
...
codex
Planen er stor, men de centrale write-veje er nu synlige. Jeg krydstjekker mod de eksisterende T9-funktioner og fitness-regler for at se, om V7 stadig har en plan-blokerende kodefejl frem for kun tekst- eller scope-spørgsmål.
exec
...
exec
/bin/bash -lc "rg -n \"function core_identity\\.is_admin|create or replace function core_identity\\.has_permission|t9_write_authorized|permission_pages|permission_tabs|role_permission_grants|pending_change_|apply\" supabase/migrations/202605*.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
supabase/migrations/20260514120007_t1_bootstrap_admins.sql:155:create or replace function core_identity.is_admin()
supabase/migrations/20260511194701_d1_5_seed_classification.sql:365:-- Smoke-tests efter apply (køres separat via execute_sql):
...
supabase/migrations/20260514150003_t7_cancellations_skeleton.sql:5:-- cancellation-handlinger + dispatcher bygges i trin 16 (sale_apply_feedback
...
supabase/migrations/20260514190200_q_class_anon_rpcs.sql:137:        v_proc := (v_mapping.internal_rpc_apply || '(uuid, jsonb, text)')::regprocedure;
supabase/migrations/20260514190200_q_class_anon_rpcs.sql:142:          'reason', 'internal_rpc_apply ' || v_mapping.internal_rpc_apply || ' findes ikke');
supabase/migrations/20260514190200_q_class_anon_rpcs.sql:153:          'entity_id', v_state.entity_id, 'reason', 'apply fejlede: ' || sqlerrm);
supabase/migrations/20260515110300_p1c_anonymize_employee_wrapper.sql:2:-- anonymize_generic_apply (P1b).
supabase/migrations/20260515110300_p1c_anonymize_employee_wrapper.sql:5:-- Pre-P1c: anonymize_employee kaldte _anonymize_employee_apply (employee-
supabase/migrations/20260515110300_p1c_anonymize_employee_wrapper.sql:8:-- generic_apply som bruger anonymization_strategies-registret.
supabase/migrations/20260515110300_p1c_anonymize_employee_wrapper.sql:41:  -- Deleger til generisk apply (P1b). Returnerer jsonb-summary; vi ignorerer
supabase/migrations/20260515110300_p1c_anonymize_employee_wrapper.sql:43:  perform core_compliance.anonymize_generic_apply('employee', p_employee_id, p_reason);
supabase/migrations/20260515110300_p1c_anonymize_employee_wrapper.sql:47:    -- Skulle ikke kunne ske; generic_apply raises P0002 først hvis row mangler.
...
supabase/migrations/20260514140001_t6_anonymization_rpcs.sql:6:--   apply_field_strategy(strategy, value) — pure helper
supabase/migrations/20260514140001_t6_anonymization_rpcs.sql:13:-- egne anonymize_<entity>-RPC'er der genbruger apply_field_strategy.
supabase/migrations/20260514140001_t6_anonymization_rpcs.sql:21:-- ─── apply_field_strategy: pure helper, anvendes af alle anonymize-RPC'er ──
supabase/migrations/20260514140001_t6_anonymization_rpcs.sql:22:create or replace function core_compliance.apply_field_strategy(
supabase/migrations/20260514140001_t6_anonymization_rpcs.sql:39:comment on function core_compliance.apply_field_strategy(text, text) is
supabase/migrations/20260514140001_t6_anonymization_rpcs.sql:42:revoke all on function core_compliance.apply_field_strategy(text, text) from public;
...
supabase/migrations/20260514140001_t6_anonymization_rpcs.sql:100:  v_new_first_name := core_compliance.apply_field_strategy(
supabase/migrations/20260514140001_t6_anonymization_rpcs.sql:103:  v_new_last_name := core_compliance.apply_field_strategy(
supabase/migrations/20260514140001_t6_anonymization_rpcs.sql:106:  v_new_email := core_compliance.apply_field_strategy(
supabase/migrations/20260515140000_r7h_anonymize_generic_apply_state_insert_fix.sql:1:-- R7h Test 3 fix: anonymize_generic_apply INSERT manglede table_schema + table_name.
supabase/migrations/20260515140000_r7h_anonymize_generic_apply_state_insert_fix.sql:4:-- anonymize_generic_apply (P1b) INSERT'er i anonymization_state, men
supabase/migrations/20260515140000_r7h_anonymize_generic_apply_state_insert_fix.sql:16:-- anonymize_generic_apply der skulle have brugt mapping.table_schema +
supabase/migrations/20260515140000_r7h_anonymize_generic_apply_state_insert_fix.sql:22:create or replace function core_compliance.anonymize_generic_apply(
...
supabase/migrations/20260518000001_t9_org_nodes.sql:69:comment on column core_identity.org_node_versions.created_by_pending_change_id is
...
supabase/migrations/20260518000001_t9_org_nodes.sql:203:  p_pending_change_id uuid
supabase/migrations/20260518000001_t9_org_nodes.sql:244:      (node_id, name, parent_id, node_type, is_active, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260518000001_t9_org_nodes.sql:246:      (v_node_id, v_name, v_parent_id, v_node_type, v_is_active, v_effective_from, null, p_pending_change_id);
supabase/migrations/20260518000001_t9_org_nodes.sql:254:      (node_id, name, parent_id, node_type, is_active, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260518000001_t9_org_nodes.sql:256:      (v_node_id, v_name, v_parent_id, v_node_type, v_is_active, v_effective_from, null, p_pending_change_id);
...
supabase/migrations/20260518000001_t9_org_nodes.sql:270:  p_pending_change_id uuid
supabase/migrations/20260518000001_t9_org_nodes.sql:312:    (node_id, name, parent_id, node_type, is_active, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260518000001_t9_org_nodes.sql:315:     false, v_effective_from, null, p_pending_change_id);
...
supabase/migrations/20260518000001_t9_org_nodes.sql:346:    raise exception 'pending_change_not_found %', p_change_id
supabase/migrations/20260518000001_t9_org_nodes.sql:351:    raise exception 'pending_change_not_approved: status=%', v_change.status
...
supabase/migrations/20260518000002_t9_org_node_closure.sql:87:-- Row-level: hver mutation triggerer rebuild. For en pending-apply der
supabase/migrations/20260520000000_t9_supplement.sql:5:--   Section B — Backdated traversal i 7 apply-handlers (Step 2)
supabase/migrations/20260520000000_t9_supplement.sql:148:-- SECTION B — Backdated traversal i 7 apply-handlers (Step 2)
...
supabase/migrations/20260520000000_t9_supplement.sql:163:  p_pending_change_id uuid
supabase/migrations/20260520000000_t9_supplement.sql:207:      (employee_id, node_id, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260520000000_t9_supplement.sql:211:      p_pending_change_id;
supabase/migrations/20260520000000_t9_supplement.sql:216:        created_by_pending_change_id = coalesce(p_pending_change_id, created_by_pending_change_id),
supabase/migrations/20260520000000_t9_supplement.sql:226:      (employee_id, node_id, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260520000000_t9_supplement.sql:228:      (v_employee_id, v_node_id, v_effective_from, v_active.effective_to, p_pending_change_id);
...
supabase/migrations/20260520000000_t9_supplement.sql:238:  p_pending_change_id uuid
...
supabase/migrations/20260520000000_t9_supplement.sql:287:  p_pending_change_id uuid
supabase/migrations/20260520000000_t9_supplement.sql:330:      (client_id, node_id, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260520000000_t9_supplement.sql:334:      p_pending_change_id;
supabase/migrations/20260520000000_t9_supplement.sql:338:        created_by_pending_change_id = coalesce(p_pending_change_id, created_by_pending_change_id),
supabase/migrations/20260520000000_t9_supplement.sql:347:      (client_id, node_id, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260520000000_t9_supplement.sql:349:      (v_client_id, v_node_id, v_effective_from, v_active.effective_to, p_pending_change_id);
...
supabase/migrations/20260520000000_t9_supplement.sql:359:  p_pending_change_id uuid
...
supabase/migrations/20260520000000_t9_supplement.sql:404:  p_pending_change_id uuid
supabase/migrations/20260520000000_t9_supplement.sql:444:      (node_id, name, parent_id, node_type, is_active, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260520000000_t9_supplement.sql:446:      (v_node_id, v_name, v_parent_id, v_node_type, v_is_active, v_effective_from, null, p_pending_change_id);
supabase/migrations/20260520000000_t9_supplement.sql:461:      (node_id, name, parent_id, node_type, is_active, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260520000000_t9_supplement.sql:465:      p_pending_change_id;
supabase/migrations/20260520000000_t9_supplement.sql:473:        created_by_pending_change_id = coalesce(p_pending_change_id, created_by_pending_change_id)
supabase/migrations/20260520000000_t9_supplement.sql:482:      (node_id, name, parent_id, node_type, is_active, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260520000000_t9_supplement.sql:484:      (v_node_id, v_name, v_parent_id, v_node_type, v_is_active, v_effective_from, v_active.effective_to, p_pending_change_id);
...
supabase/migrations/20260520000000_t9_supplement.sql:496:  p_pending_change_id uuid
supabase/migrations/20260520000000_t9_supplement.sql:536:        created_by_pending_change_id = coalesce(p_pending_change_id, created_by_pending_change_id)
supabase/migrations/20260520000000_t9_supplement.sql:545:      (node_id, name, parent_id, node_type, is_active, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260520000000_t9_supplement.sql:547:      (v_node_id, v_active.name, v_active.parent_id, v_active.node_type, false, v_effective_from, v_active.effective_to, p_pending_change_id);
...
supabase/migrations/20260520000000_t9_supplement.sql:559:  p_pending_change_id uuid
supabase/migrations/20260520000000_t9_supplement.sql:607:        created_by_pending_change_id = coalesce(p_pending_change_id, created_by_pending_change_id)
supabase/migrations/20260520000000_t9_supplement.sql:615:      (node_id, name, parent_id, node_type, is_active, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260520000000_t9_supplement.sql:617:      (v_node_id, v_active.name, v_active.parent_id, 'team', false, v_effective_from, v_active.effective_to, p_pending_change_id);
...
supabase/migrations/20260515130300_r7d_is_active_status_alignment.sql:121:  v_row := core_identity._anonymize_employee_apply(p_employee_id, v_mapping.field_strategies, p_reason);
supabase/migrations/20260519000000_m1_t9_superadmin_permissions.sql:16:-- T9-fundament-supplement (PR #39) pending_change_approve/undo
supabase/migrations/20260515110200_p1b_anonymize_generic_apply.sql:1:-- P1b: anonymize_generic_apply — generisk anonymization der bruger
supabase/migrations/20260515110200_p1b_anonymize_generic_apply.sql:5:-- Den eksisterende _anonymize_employee_apply var employee-specifik og hardkodede
supabase/migrations/20260515110200_p1b_anonymize_generic_apply.sql:6:-- strategi-håndtering. P1b giver en generisk apply der virker for enhver
supabase/migrations/20260515110200_p1b_anonymize_generic_apply.sql:24:create or replace function core_compliance.anonymize_generic_apply(
supabase/migrations/20260515110200_p1b_anonymize_generic_apply.sql:182:comment on function core_compliance.anonymize_generic_apply(text, uuid, text) is
supabase/migrations/20260515110200_p1b_anonymize_generic_apply.sql:185:revoke all on function core_compliance.anonymize_generic_apply(text, uuid, text) from public;
...
supabase/migrations/20260518000007_t9_public_wrapper_rpcs.sql:75:  -- Pre-check: verificér at det er team-knude (apply-handler re-validerer).
...
supabase/migrations/20260518000011_t9_classify.sql:69:  ('core_identity', 'employee_node_placements', 'applied_at', 'operationel', 'none', 'time_based', '{"max_days":2555}'::jsonb, null, 'apply-timestamp'),
supabase/migrations/20260518000011_t9_classify.sql:70:  ('core_identity', 'employee_node_placements', 'created_by_pending_change_id', 'operationel', 'none', 'time_based', '{"max_days":2555}'::jsonb, null, 'FK pending'),
...
supabase/migrations/20260518100000_t9_fundament_supplement.sql:22:-- - KRITISK 4 Backdated effective_from guards på 5 apply-handlers
...
supabase/migrations/20260518100000_t9_fundament_supplement.sql:165:-- som apply-handlers' dispatcher per change_type. Mappingen er udtømmende;
...
supabase/migrations/20260518100000_t9_fundament_supplement.sql:195:    raise exception 'pending_change_not_found %', p_change_id
supabase/migrations/20260518100000_t9_fundament_supplement.sql:200:    raise exception 'pending_change_wrong_status: % (expected pending)', v_change.status
supabase/migrations/20260518100000_t9_fundament_supplement.sql:225:    raise exception 'pending_change_self_approve_forbidden'
...
supabase/migrations/20260518100000_t9_fundament_supplement.sql:257:create or replace function core_identity.pending_change_undo(
supabase/migrations/20260518100000_t9_fundament_supplement.sql:274:    raise exception 'pending_change_not_found %', p_change_id
supabase/migrations/20260518100000_t9_fundament_supplement.sql:279:    raise exception 'pending_change_wrong_status: % (expected approved)', v_change.status
...
supabase/migrations/20260518100000_t9_fundament_supplement.sql:310:  perform set_config('stork.change_reason', 'pending_change_undo', true);
supabase/migrations/20260518100000_t9_fundament_supplement.sql:319:revoke execute on function core_identity.pending_change_undo(uuid) from public, anon;
...
supabase/migrations/20260518000000_t9_pending_changes.sql:15:--   pending_change_approve(change_id)
supabase/migrations/20260518000000_t9_pending_changes.sql:17:--   pending_change_undo(change_id)
...
supabase/migrations/20260518000000_t9_pending_changes.sql:161:-- ─── RPC: pending_change_approve ─────────────────────────────────────────
supabase/migrations/20260518000000_t9_pending_changes.sql:162:create or replace function core_identity.pending_change_approve(
supabase/migrations/20260518000000_t9_pending_changes.sql:186:    raise exception 'pending_change_not_found %', p_change_id
supabase/migrations/20260518000000_t9_pending_changes.sql:191:    raise exception 'pending_change_wrong_status: % (expected pending)', v_change.status
supabase/migrations/20260518000000_t9_pending_changes.sql:196:    raise exception 'pending_change_self_approve_forbidden'
supabase/migrations/20260518000000_t9_pending_changes.sql:211:  perform set_config('stork.change_reason', 'pending_change_approve', true);
supabase/migrations/20260518000000_t9_pending_changes.sql:223:comment on function core_identity.pending_change_approve(uuid) is
supabase/migrations/20260518000000_t9_pending_changes.sql:226:revoke execute on function core_identity.pending_change_approve(uuid) from public, anon;
supabase/migrations/20260518000000_t9_pending_changes.sql:228:-- ─── RPC: pending_change_undo ────────────────────────────────────────────
supabase/migrations/20260518000000_t9_pending_changes.sql:229:create or replace function core_identity.pending_change_undo(
supabase/migrations/20260518000000_t9_pending_changes.sql:245:    raise exception 'pending_change_not_found %', p_change_id
supabase/migrations/20260518000000_t9_pending_changes.sql:250:    raise exception 'pending_change_wrong_status: % (expected approved)', v_change.status
supabase/migrations/20260518000000_t9_pending_changes.sql:261:  perform set_config('stork.change_reason', 'pending_change_undo', true);
supabase/migrations/20260518000000_t9_pending_changes.sql:271:comment on function core_identity.pending_change_undo(uuid) is
supabase/migrations/20260518000000_t9_pending_changes.sql:274:revoke execute on function core_identity.pending_change_undo(uuid) from public, anon;
...
supabase/migrations/20260518000000_t9_pending_changes.sql:296:    raise exception 'pending_change_not_found %', p_change_id
supabase/migrations/20260518000000_t9_pending_changes.sql:300:  -- V6 central apply-gate: due-check.
supabase/migrations/20260518000000_t9_pending_changes.sql:302:    raise exception 'pending_change_not_approved: status=%', v_change.status
...
supabase/migrations/20260514180000_g028_classify_anonymization_dispatcher_columns.sql:29:  ('core_compliance', 'anonymization_mappings', 'internal_rpc_apply',
supabase/migrations/20260514180000_g028_classify_anonymization_dispatcher_columns.sql:31:   'fuldt kvalificeret RPC-navn (signatur uuid, jsonb, text) til pure apply — bruges af replay-dispatcher med snapshot (C003)'),
supabase/migrations/20260514180000_g028_classify_anonymization_dispatcher_columns.sql:45:      ('core_compliance', 'anonymization_mappings', 'internal_rpc_apply'),
...
supabase/migrations/20260518000004_t9_client_node_placements.sql:88:  p_pending_change_id uuid
supabase/migrations/20260518000004_t9_client_node_placements.sql:115:    (client_id, node_id, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260518000004_t9_client_node_placements.sql:117:    (v_client_id, v_node_id, v_effective_from, null, p_pending_change_id);
...
supabase/migrations/20260518000004_t9_client_node_placements.sql:125:  p_pending_change_id uuid
...
supabase/migrations/20260518000004_t9_client_node_placements.sql:168:    raise exception 'pending_change_not_found %', p_change_id
supabase/migrations/20260518000004_t9_client_node_placements.sql:173:    raise exception 'pending_change_not_approved: status=%', v_change.status
...
supabase/migrations/20260515130000_r7a_regprocedure_callable_fix.sql:14:-- 1. core_compliance.anonymize_generic_apply (pg_proc)
supabase/migrations/20260515130000_r7a_regprocedure_callable_fix.sql:25:-- ─── 1. anonymize_generic_apply ────────────────────────────────────────────
supabase/migrations/20260515130000_r7a_regprocedure_callable_fix.sql:26:create or replace function core_compliance.anonymize_generic_apply(
supabase/migrations/20260515130000_r7a_regprocedure_callable_fix.sql:272:        v_proc := (v_mapping.internal_rpc_apply || '(uuid, jsonb, text)')::regprocedure;
supabase/migrations/20260515130000_r7a_regprocedure_callable_fix.sql:277:          'reason', 'internal_rpc_apply ' || v_mapping.internal_rpc_apply || ' findes ikke');
supabase/migrations/20260515130000_r7a_regprocedure_callable_fix.sql:293:          'entity_id', v_state.entity_id, 'reason', 'apply fejlede: ' || sqlerrm);
supabase/migrations/20260518000003_t9_employee_node_placements.sql:1:-- Trin 9 / §4 trin 9 Step 4: employee_node_placements + apply-handlers.
...
supabase/migrations/20260518000003_t9_employee_node_placements.sql:65:  p_pending_change_id uuid
supabase/migrations/20260518000003_t9_employee_node_placements.sql:104:    (employee_id, node_id, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260518000003_t9_employee_node_placements.sql:106:    (v_employee_id, v_node_id, v_effective_from, null, p_pending_change_id);
...
supabase/migrations/20260518000003_t9_employee_node_placements.sql:115:  p_pending_change_id uuid
...
supabase/migrations/20260518000003_t9_employee_node_placements.sql:153:  p_pending_change_id uuid
supabase/migrations/20260518000003_t9_employee_node_placements.sql:199:    (node_id, name, parent_id, node_type, is_active, effective_from, effective_to, created_by_pending_change_id)
supabase/migrations/20260518000003_t9_employee_node_placements.sql:202:     false, v_effective_from, null, p_pending_change_id);
...
supabase/migrations/20260518000003_t9_employee_node_placements.sql:241:    raise exception 'pending_change_not_found %', p_change_id
supabase/migrations/20260518000003_t9_employee_node_placements.sql:246:    raise exception 'pending_change_not_approved: status=%', v_change.status
...
supabase/migrations/20260514190400_q_hr_rpcs.sql:128:  v_row := core_identity._anonymize_employee_apply(p_employee_id, v_mapping.field_strategies, p_reason);
...
supabase/migrations/20260515120000_p2_anonymization_mapping_lifecycle.sql:19:-- (kraever UI-aktivering pre-cutover for at koeren anonymize_generic_apply).
supabase/migrations/20260515120000_p2_anonymization_mapping_lifecycle.sql:21:-- anonymize_generic_apply opdateres til at kraeve status='active' paa
supabase/migrations/20260515120000_p2_anonymization_mapping_lifecycle.sql:117:  p_internal_rpc_apply text,
supabase/migrations/20260515120000_p2_anonymization_mapping_lifecycle.sql:146:    internal_rpc_anonymize, internal_rpc_apply, is_active, status
supabase/migrations/20260515120000_p2_anonymization_mapping_lifecycle.sql:150:    p_internal_rpc_anonymize, p_internal_rpc_apply, false, 'draft'
supabase/migrations/20260515120000_p2_anonymization_mapping_lifecycle.sql:157:    internal_rpc_apply = excluded.internal_rpc_apply
supabase/migrations/20260515120000_p2_anonymization_mapping_lifecycle.sql:322:-- ─── Opdatér anonymize_generic_apply til at kraeve status='active' ────────
supabase/migrations/20260515120000_p2_anonymization_mapping_lifecycle.sql:323:create or replace function core_compliance.anonymize_generic_apply(
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:20:-- 2. Split anonymize_employee i 3 RPC'er + 1 intern apply-helper
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:39:  add column internal_rpc_apply text,
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:45:comment on column core_compliance.anonymization_mappings.internal_rpc_apply is
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:46:  'Fuldt kvalificeret RPC-navn (signatur uuid, jsonb, text) til pure apply: UPDATE only (ingen INSERT state). Bruges af replay-dispatcher med snapshot-strategies.';
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:61:       internal_rpc_apply = 'core_identity._anonymize_employee_apply',
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:70:  alter column internal_rpc_apply set not null;
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:73:-- Step 2: Split anonymize_employee i apply + admin + internal (service-role)
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:76:-- _anonymize_employee_apply: pure UPDATE, ingen state-INSERT, ingen permission-check.
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:78:create or replace function core_identity._anonymize_employee_apply(
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:108:  v_new_first_name := core_compliance.apply_field_strategy(p_strategies->>'first_name', null);
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:109:  v_new_last_name := core_compliance.apply_field_strategy(p_strategies->>'last_name', null);
...
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:113:  perform set_config('stork.change_reason', 'anonymization_apply: ' || p_reason, true);
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:127:comment on function core_identity._anonymize_employee_apply(uuid, jsonb, text) is
...
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:159:-- anonymize_employee: admin-vej. is_admin strict, læser live mapping, kalder apply + log_state.
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:190:  v_row := core_identity._anonymize_employee_apply(p_employee_id, v_mapping.field_strategies, p_reason);
...
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:234:  v_row := core_identity._anonymize_employee_apply(p_employee_id, v_mapping.field_strategies, p_reason);
...
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:437:    -- Master-row har PII tilbage → re-apply med SNAPSHOT (ikke live mapping)
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:440:        v_proc := (v_mapping.internal_rpc_apply || '(uuid, jsonb, text)')::regprocedure;
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:445:          'reason', 'internal_rpc_apply ' || v_mapping.internal_rpc_apply || ' findes ikke'
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:460:          'entity_id', v_state.entity_id, 'reason', 'apply fejlede: ' || sqlerrm
supabase/migrations/20260514170004_c002_c003_anonymization_dispatcher.sql:481:  'C003 + G010: idempotent replay via snapshot. Bruger anonymization_mappings-dispatcher (generisk for alle entity-typer). INGEN INSERT i anonymization_state — apply-only via internal_rpc_apply.';

exec
/bin/bash -lc "rg -n \"legacy-is-active-readers|LEGACY_IS_ACTIVE_EXEMPT_FUNCTIONS|status = 'active'|is_active = true|CREATE TABLE|no-dedup-key|set-config|set_config|migration-on-conflict|db-test-tx-wrap\" scripts/fitness.mjs" in /home/mathias/stork-2.0
 succeeded in 0ms:
16:// eksplicit opt-out med `-- no-dedup-key: <reason>` i CREATE TABLE-blokken.
...
174:// Migrationsfiler der er undtaget fra set-config-discipline-check.
393:// Hvis migration KUN udfører DDL (CREATE TABLE, ALTER, CREATE TRIGGER,
410:    const hasSourceType = /set_config\s*\(\s*'stork\.source_type'/i.test(cleaned);
411:    const hasChangeReason = /set_config\s*\(\s*'stork\.change_reason'/i.test(cleaned);
419:  return { name: "migration-set-config-discipline", violations };
422:// D6 check 2: nye CREATE TABLE skal enten have dedup_key text-kolonne
423:// ELLER eksplicit "-- no-dedup-key: <reason>" comment i migration-filen.
431:    // Strip kommentarer så CREATE TABLE i docs ikke fanger.
434:    // foran CREATE TABLE — markeret med "-- no-dedup-key: <reason>").
435:    const optOutRe = /--\s*no-dedup-key:\s*\S+/i;
448:          `${rel(path)}: ${qualified} mangler enten dedup_key-kolonne eller "-- no-dedup-key: <reason>"-marker`,
534:      if (!/set_config\s*\(\s*'stork\.change_reason'/i.test(callBody)) {
539:          `${rel(path)}:${line}: cron.schedule mangler set_config('stork.change_reason', ...) i command-body`,
628:  // Find alle CREATE TABLE i core_*-schemas
662:  // Verificér at allowlist-entries faktisk eksisterer i CREATE TABLE
667:        `${qualified}: er i AUDIT_EXEMPT_SNAPSHOT_TABLES men ingen CREATE TABLE-statement findes — fjern fra allowlist`,
737:  return { name: "migration-on-conflict-discipline", violations };
762:      WHERE status = 'active' AND is_active = true
827:// Pattern: hver function-body / cron-body med `is_active = true` skal
828:// også indeholde `status = 'active'`. False-positives accepteret hvis
...
923:  return { name: "db-test-tx-wrap-on-immutable-insert", violations };

exec
/bin/bash -lc "sed -n '1,170p' supabase/migrations/20260514130000_t2_superadmin_floor.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc "sed -n '326,395p' supabase/migrations/20260518000001_t9_org_nodes.sql && sed -n '151,224p' supabase/migrations/20260518000004_t9_client_node_placements.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc "rg -n \"grant execute on function core_identity\\.(client_node_place|pending_change_|org_node_upsert|employee_place|client_node_close)|revoke execute on function core_identity\\.(client_node_place|pending_change_)\" supabase/migrations" in /home/mathias/stork-2.0
 succeeded in 0ms:
supabase/migrations/20260518000004_t9_client_node_placements.sql:220:revoke execute on function core_identity.pending_change_apply(uuid) from public, anon;
supabase/migrations/20260518000003_t9_employee_node_placements.sql:289:revoke execute on function core_identity.pending_change_apply(uuid) from public, anon;
supabase/migrations/20260518000000_t9_pending_changes.sql:159:revoke execute on function core_identity.pending_change_request(text, uuid, jsonb, date) from public, anon, authenticated;
...
exec
/bin/bash -lc "sed -n '393,455p' scripts/fitness.mjs && sed -n '827,895p' scripts/fitness.mjs && sed -n '901,924p' scripts/fitness.mjs" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc "sed -n '1,380p' supabase/migrations/20260518000000_t9_pending_changes.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc "rg -n \"grant execute on all functions|alter default privileges|grant usage on schema core_identity|grant execute on function core_identity\\.(org_node_upsert|team_close|employee_place|client_node_place|client_node_close|pending_change_approve|pending_change_apply)\" supabase/migrations" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc "sed -n '220,330p' supabase/migrations/20260518100000_t9_fundament_supplement.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc "sed -n '1,150p' supabase/migrations/20260518000009_t9_migrate_role_page_permissions.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc "sed -n '180,240p' supabase/migrations/20260518000010_t9_seed_owners.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc "sed -n '120,220p' supabase/migrations/20260518100000_t9_fundament_supplement.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc 'rg -n "permission_pages_insert|permission_tabs_insert|role_permission_grants_insert|t9_write_authorized|grant insert|grant update" supabase/migrations/20260518*.sql supabase/migrations/20260520*.sql' in /home/mathias/stork-2.0
 succeeded in 0ms:
supabase/migrations/20260518100000_t9_fundament_supplement.sql:16:-- - 6 write-tabeller får INSERT + UPDATE policies med t9_write_authorized-check
supabase/migrations/20260518100000_t9_fundament_supplement.sql:17:-- - 11 write-RPCs får perform set_config('stork.t9_write_authorized', 'true', true)
...
exec
/bin/bash -lc "sed -n '1,120p' supabase/migrations/20260514120006_t1_data_field_definitions.sql" in /home/mathias/stork-2.0
 exited 2 in 0ms:
sed: can't read supabase/migrations/20260514120006_t1_data_field_definitions.sql: No such file or directory

exec
/bin/bash -lc "rg -n \"create table core_compliance\\.data_field_definitions|category text|retention_type|pii_level\" supabase/migrations/202605141200*.sql supabase/migrations/20260515110100_p1a_anonymization_strategies.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
supabase/migrations/20260514120006_t1_audit_filter_values.sql:3:-- Master-plan §1.3: pii_level='direct' hashes til sha256 før audit-skrivning.
supabase/migrations/20260514120006_t1_audit_filter_values.sql:47:  -- Walker hver kolonne i p_values; hashes hvis pii_level='direct'.
supabase/migrations/20260514120006_t1_audit_filter_values.sql:49:    select column_name, pii_level
supabase/migrations/20260514120006_t1_audit_filter_values.sql:54:      if v_def.pii_level = 'direct' then
supabase/migrations/20260514120006_t1_audit_filter_values.sql:89:  'Master-plan §1.3 PII-filter. pii_level=direct hashes til sha256:<hex>. LENIENT-default — strict via stork.audit_filter_strict=true.';
...
supabase/migrations/20260514120005_t1_data_field_definitions.sql:17:  pii_level text not null check (pii_level in ('none', 'indirect', 'direct')),
...
supabase/migrations/20260514120005_t1_data_field_definitions.sql:40:  where pii_level = 'direct';
...
supabase/migrations/20260514120005_t1_data_field_definitions.sql:143:  p_pii_level text,
...
supabase/migrations/20260514120005_t1_data_field_definitions.sql:178:    pii_level = excluded.pii_level,
supabase/migrations/20260514120005_t1_data_field_definitions.sql:179:    retention_type = excluded.retention_type,

exec
/bin/bash -lc "sed -n '120,260p' supabase/migrations/20260514120006_t1_data_field_definitions.sql" in /home/mathias/stork-2.0
 exited 2 in 0ms:
sed: can't read supabase/migrations/20260514120006_t1_data_field_definitions.sql: No such file or directory
...
exec
/bin/bash -lc "sed -n '120,220p' supabase/migrations/20260514120005_t1_data_field_definitions.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
  using (current_setting('stork.allow_data_field_definitions_write', true) = 'true')
  with check (current_setting('stork.allow_data_field_definitions_write', true) = 'true');
...
exec
/bin/bash -lc "rg -n \"permanent|is_permanent_allowed|retention_type.*permanent|alter table core_compliance\\.data_field_definitions\" supabase/migrations" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
supabase/migrations/20260516200000_h024_test_artifact_cleanup.sql:4:-- + DISABLE/ENABLE TRIGGER pattern. One-shot pre-cutover; ingen permanent
...
supabase/migrations/20260514180500_d1_d2_drop_legal_convert_rows.sql:27:alter table core_compliance.data_field_definitions
...
supabase/migrations/20260514180500_d1_d2_drop_legal_convert_rows.sql:48:alter table core_compliance.data_field_definitions
...
supabase/migrations/20260514180500_d1_d2_drop_legal_convert_rows.sql:53:alter table core_compliance.data_field_definitions
...
exec
/bin/bash -lc "rg -n \"create or replace function core_compliance\\.is_permanent_allowed\" supabase/migrations" in /home/mathias/stork-2.0
 succeeded in 0ms:
supabase/migrations/20260515110100_p1a_anonymization_strategies.sql:231:create or replace function core_compliance.is_permanent_allowed(
supabase/migrations/20260514180400_d1b_is_permanent_allowed.sql:14:create or replace function core_compliance.is_permanent_allowed(

exec
/bin/bash -lc "sed -n '1,80p' supabase/migrations/20260514180400_d1b_is_permanent_allowed.sql && sed -n '230,265p' supabase/migrations/20260515110100_p1a_anonymization_strategies.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc "rg -n \"retention_type.*'permanent'|is_permanent_allowed|allowlist\" supabase/migrations/2026051512*.sql supabase/migrations/2026051418*.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
supabase/migrations/20260514180600_d1c_validate_permanent_classification.sql:3:-- is_permanent_allowed-allowlist for retention_type='permanent'.
supabase/migrations/20260514180600_d1c_validate_permanent_classification.sql:8:-- på at sætte 'permanent' på kolonner uden for is_permanent_allowed-listen.
supabase/migrations/20260514180600_d1c_validate_permanent_classification.sql:11:-- is_permanent_allowed-funktionens VALUES-blok i ny migration).
...
supabase/migrations/20260514180400_d1b_is_permanent_allowed.sql:4:-- retention_type='permanent' må kun bruges på system-meta-kolonner:
supabase/migrations/20260514180400_d1b_is_permanent_allowed.sql:14:create or replace function core_compliance.is_permanent_allowed(
supabase/migrations/20260514180400_d1b_is_permanent_allowed.sql:41:    ) as allowlist(t_schema, t_name, t_column)
supabase/migrations/20260514180400_d1b_is_permanent_allowed.sql:42:    where allowlist.t_schema = p_table_schema
supabase/migrations/20260514180400_d1b_is_permanent_allowed.sql:43:      and allowlist.t_name = p_table_name
supabase/migrations/20260514180400_d1b_is_permanent_allowed.sql:44:      and (allowlist.t_column is null or allowlist.t_column = p_column_name)
supabase/migrations/20260514180400_d1b_is_permanent_allowed.sql:48:comment on function core_compliance.is_permanent_allowed(text, text, text) is
supabase/migrations/20260514180400_d1b_is_permanent_allowed.sql:49:  'D1b: IMMUTABLE-allowlist for retention_type=permanent. t_column=null betyder "alle kolonner i tabellen". Allowlist-aendringer kraever kode-commit + review (master-plan rettelse 29).';
supabase/migrations/20260514180400_d1b_is_permanent_allowed.sql:51:revoke all on function core_compliance.is_permanent_allowed(text, text, text) from public;
supabase/migrations/20260514180400_d1b_is_permanent_allowed.sql:52:grant execute on function core_compliance.is_permanent_allowed(text, text, text) to authenticated;
...
supabase/migrations/20260514180500_d1_d2_drop_legal_convert_rows.sql:56:      (retention_type = 'permanent' and retention_value is null)
supabase/migrations/20260514180000_g028_classify_anonymization_dispatcher_columns.sql:13:-- - C001-disciplin: retention_type NOT NULL. Konfiguration-felter → 'permanent'.
supabase/migrations/20260515120000_p2_anonymization_mapping_lifecycle.sql:171:-- forsoege en dry-run-iteration over PII-allowlist (uden faktisk UPDATE).

exec
/bin/bash -lc "rg -n \"create schema.*extensions|digest\\(\" supabase/migrations" in /home/mathias/stork-2.0
exec
/bin/bash -lc "sed -n '1,115p' supabase/migrations/20260514120006_t1_audit_filter_values.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
supabase/migrations/20260514140001_t6_anonymization_rpcs.sql:33:    when 'hash' then 'sha256:' || encode(extensions.digest(coalesce(p_value, ''), 'sha256'), 'hex')
supabase/migrations/20260514140001_t6_anonymization_rpcs.sql:34:    when 'hash_email' then 'anon-' || substr(encode(extensions.digest(coalesce(p_value, ''), 'sha256'), 'hex'), 1, 16) || '@anonymized.local'
supabase/migrations/20260514120006_t1_audit_filter_values.sql:60:            encode(extensions.digest((p_values->>v_def.column_name)::text, 'sha256'), 'hex')
supabase/migrations/20260515110100_p1a_anonymization_strategies.sql:208:    encode(extensions.digest(coalesce(p_value, '') || ':' || coalesce(p_entity_id, ''), 'sha256'), 'hex'),
supabase/migrations/20260515110350_p1a_fix_strategy_completeness.sql:37:    extensions.digest(coalesce(p_value, '') || ':' || coalesce(p_entity_id, ''), 'sha256'),
supabase/migrations/20260511213009_d5_clients.sql:201:            to_jsonb('sha256:' || encode(extensions.digest(v_value::text, 'sha256'), 'hex'))
supabase/migrations/20260511213009_d5_clients.sql:225:            to_jsonb('sha256:' || encode(extensions.digest(v_field_value::text, 'sha256'), 'hex'))
supabase/migrations/20260511195819_d2_audit_filter_values.sql:23:-- pgcrypto's digest() ligger i extensions schema. Med SET search_path = ''
supabase/migrations/20260511195819_d2_audit_filter_values.sql:24:-- fully-qualifies vi som extensions.digest().
supabase/migrations/20260511195819_d2_audit_filter_values.sql:87:            to_jsonb('sha256:' || encode(extensions.digest(v_value::text, 'sha256'), 'hex'))
supabase/migrations/20260514150005_t7_lock_pipeline.sql:71:    extensions.digest(v_inputs::text, 'sha256'),
...
exec
/bin/bash -lc "sed -n '145,190p' supabase/migrations/20260514120007_t1_bootstrap_admins.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
  select e.id
  from core_identity.employees e
...
exec
/bin/bash -lc "sed -n '25,55p' supabase/migrations/20260514120002_t1_helpers_stubs.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc "rg -n \"create or replace function core_identity\\.is_admin\" supabase/migrations/20260518*.sql supabase/migrations/20260520*.sql docs/coordination/trin-10-plan.md" in /home/mathias/stork-2.0
 exited 1 in 0ms:
exec
/bin/bash -lc "rg -n \"cron\\.schedule|pending_changes_apply_due|pending_change_apply\\(\" supabase/migrations/20260518000000_t9_pending_changes.sql supabase/migrations/20260518*.sql supabase/migrations/20260520*.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
supabase/migrations/20260518000001_t9_org_nodes.sql:330:create or replace function core_identity.pending_change_apply(
supabase/migrations/20260518000001_t9_org_nodes.sql:388:revoke execute on function core_identity.pending_change_apply(uuid) from public, anon;
supabase/migrations/20260518000000_t9_pending_changes.sql:19:--   pending_change_apply(change_id)
...
exec
/bin/bash -lc "sed -n '300,380p' supabase/migrations/20260518000000_t9_pending_changes.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
  -- V6 central apply-gate: due-check.
  if v_change.status <> 'approved' then
    raise exception 'pending_change_not_approved: status=%', v_change.status
...
exec
/bin/bash -lc "sed -n '393,432p' supabase/migrations/20260518000000_t9_pending_changes.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc "rg -n \"create or replace function core_identity\\.pending_change_apply|stork\\.t9_write_authorized|perform core_identity\\._apply_client_place|case v_change\\.change_type\" supabase/migrations/20260518100000_t9_fundament_supplement.sql supabase/migrations/20260520000000_t9_supplement.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
supabase/migrations/20260518100000_t9_fundament_supplement.sql:17:-- - 11 write-RPCs får perform set_config('stork.t9_write_authorized', 'true', true)
supabase/migrations/20260518100000_t9_fundament_supplement.sql:44:-- Begge kræver current_setting('stork.t9_write_authorized', true) = 'true'.
supabase/migrations/20260518100000_t9_fundament_supplement.sql:51:  with check (current_setting('stork.t9_write_authorized', true) = 'true');
...
supabase/migrations/20260518100000_t9_fundament_supplement.sql:137:  using (current_setting('stork.t9_write_authorized', true) = 'true');
supabase/migrations/20260518100000_t9_fundament_supplement.sql:143:--   2. perform set_config('stork.t9_write_authorized', 'true', true)
supabase/migrations/20260518100000_t9_fundament_supplement.sql:206:  case v_change.change_type
supabase/migrations/20260518100000_t9_fundament_supplement.sql:237:  perform set_config('stork.t9_write_authorized', 'true', true);
supabase/migrations/20260518100000_t9_fundament_supplement.sql:290:  case v_change.change_type
supabase/migrations/20260518100000_t9_fundament_supplement.sql:308:  perform set_config('stork.t9_write_authorized', 'true', true);
supabase/migrations/20260518100000_t9_fundament_supplement.sql:344:  perform set_config('stork.t9_write_authorized', 'true', true);
supabase/migrations/20260518100000_t9_fundament_supplement.sql:377:  perform set_config('stork.t9_write_authorized', 'true', true);
supabase/migrations/20260518100000_t9_fundament_supplement.sql:408:  perform set_config('stork.t9_write_authorized', 'true', true);
...
exec
/bin/bash -lc "sed -n '330,575p' supabase/migrations/20260518100000_t9_fundament_supplement.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
declare
  v_updater uuid;
begin
...
exec
/bin/bash -lc "rg -n \"t10_client_node_placements_fk|DELETE af klient|on delete restrict|delete from core_identity\\.clients|RESTRICT\" docs/coordination/trin-10-plan.md" in /home/mathias/stork-2.0
 succeeded in 0ms:
19:| 3   | MELLEM   | T10.15             | Smoke-test dækker FK-eksistens + ON DELETE RESTRICT, men ikke det vigtigste forretningskrav (krav-dok §2.5.2: inaktiv klient kan ikke vælges).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | **ACCEPT.** Ny separat smoke-test `t10_client_active_check.sql` med 4 scenarier: aktiv place success, inaktiv place rejection (22023), pending-mens-aktiv + deaktiver + apply rejection (P0001), close virker på inaktiv klient.                                                                                                                                                                                   | T10.15 + ny test-fil          |
39:| 6   | KRITISK             | T10.15 `t10_client_node_placements_fk.sql`      | Smoke-test INSERT'er i `core_identity.client_node_placements` som er på `TX_WRAP_REQUIRED_FOR_TEST_INSERT` (`scripts/fitness.mjs:110`). Fitness-check `db-test-tx-wrap-on-immutable-insert` (`:901-924`) kræver `begin;` + `rollback;` på linje-niveau.                                                                                                | Code-validering  | **ACCEPT.** Eksplicit `begin;` + `rollback;` wrap-pattern i T10.15's FK-test specifikation. T10.7a's fixture-INSERT i T9-tests sker indenfor eksisterende BEGIN/ROLLBACK (verificeret: `t9_placements.sql:9` + `:213`, `t9_backdated_historical_traversal.sql:9` + `:311`).                                                                                                                            |
689:      on delete restrict;
693:- **Migration-fil:** `supabase/migrations/<ts>_t10_client_node_placements_fk.sql`
1316:  | `supabase/tests/smoke/t10_client_node_placements_fk.sql` | FK virker: INSERT med ikke-eksisterende client_id fejler. DELETE af klient med åbne placements fejler RESTRICT. **V6 (Code-validering fund #6):** Test SKAL være `begin;` + `rollback;`-wrapped (linje-niveau) — `core_identity.client_node_placements` er på `TX_WRAP_REQUIRED_FOR_TEST_INSERT` (`scripts/fitness.mjs:110`). Fitness-check `db-test-tx-wrap-on-immutable-insert` blokerer ellers.                                                                                                                                                                                                                                                                                                                                                               |
1367:| `t10_client_node_placements_fk.sql` | FK afviser ikke-eksisterende client_id; ON DELETE RESTRICT                                                            | grøn             |
1492:- T10.15 `t10_client_node_placements_fk.sql`: eksplicit `begin;` + `rollback;` wrap — `client_node_placements` på `TX_WRAP_REQUIRED_FOR_TEST_INSERT` (Code-validering #6).
...
exec
/bin/bash -lc 'rg -n "set role|request.jwt.claim|auth.uid|auth.jwt|begin;|rollback;|delete from core_identity|insert into core_identity" supabase/tests/smoke/t9_placements.sql supabase/tests/smoke/t9_backdated_historical_traversal.sql supabase/tests/smoke/*.sql' in /home/mathias/stork-2.0
 succeeded in 0ms:
supabase/tests/smoke/r7a_break_glass_execute_e2e.sql:14:begin;
...
supabase/tests/smoke/r7a_break_glass_execute_e2e.sql:89:rollback;
supabase/tests/smoke/r7a_anonymize_generic_apply_e2e.sql:17:begin;
...
supabase/tests/smoke/r7a_anonymize_generic_apply_e2e.sql:83:rollback;
supabase/tests/smoke/p1a_anonymization_strategies.sql:13:begin;
supabase/tests/smoke/p1a_anonymization_strategies.sql:66:rollback;
supabase/tests/smoke/r3_commission_snapshots_immutability.sql:14:begin;
supabase/tests/smoke/r3_commission_snapshots_immutability.sql:81:rollback;
supabase/tests/smoke/t9_public_wrapper_rpcs.sql:17:begin;
...
supabase/tests/smoke/t9_public_wrapper_rpcs.sql:105:rollback;
supabase/tests/smoke/t9_placements.sql:9:begin;
...
supabase/tests/smoke/t9_placements.sql:128:    reset role;
supabase/tests/smoke/t9_placements.sql:131:    reset role;
supabase/tests/smoke/t9_placements.sql:213:rollback;
supabase/tests/smoke/t9_read_gates.sql:16:begin;
...
supabase/tests/smoke/t9_read_gates.sql:146:rollback;
...
supabase/tests/smoke/t9_grants_and_helpers.sql:8:begin;
...
supabase/tests/smoke/t9_grants_and_helpers.sql:162:rollback;
supabase/tests/smoke/t9_org_nodes.sql:23:begin;
...
supabase/tests/smoke/t9_org_nodes.sql:204:    reset role;
supabase/tests/smoke/t9_org_nodes.sql:207:    reset role;
...
supabase/tests/smoke/t9_org_nodes.sql:291:rollback;
supabase/tests/smoke/t9_org_node_closure.sql:11:begin;
...
supabase/tests/smoke/t9_org_node_closure.sql:122:rollback;
...
supabase/tests/smoke/has_permission_admin_grant.sql:8:  v_mg_auth_uid uuid;
supabase/tests/smoke/has_permission_admin_grant.sql:10:  select auth_user_id into v_mg_auth_uid
supabase/tests/smoke/has_permission_admin_grant.sql:12:  if v_mg_auth_uid is null then
...
supabase/tests/smoke/t9_placements.sql:131:    reset role;
supabase/tests/smoke/t9_placements.sql:213:rollback;
supabase/tests/smoke/r7a_retention_cleanup_cron_e2e.sql:15:begin;
...
supabase/tests/smoke/r7a_retention_cleanup_cron_e2e.sql:74:rollback;
supabase/tests/smoke/t9_pending_changes.sql:19:begin;
...
supabase/tests/smoke/t9_pending_changes.sql:59:    reset role;
supabase/tests/smoke/t9_pending_changes.sql:62:    reset role;
...
supabase/tests/smoke/t9_pending_changes.sql:184:rollback;
supabase/tests/smoke/r7a_replay_anonymization_e2e.sql:12:begin;
...
supabase/tests/smoke/r7a_replay_anonymization_e2e.sql:85:rollback;
supabase/tests/smoke/t9_backdated_historical_traversal.sql:9:begin;
...
supabase/tests/smoke/t9_backdated_historical_traversal.sql:130:  delete from core_identity.employee_node_placements where employee_id = v_emp_a_id;
...
supabase/tests/smoke/t9_backdated_historical_traversal.sql:199:  delete from core_identity.org_node_versions where node_id = v_node_team_a_id;
...
supabase/tests/smoke/t9_backdated_historical_traversal.sql:215:  delete from core_identity.org_node_versions where node_id = v_node_team_a_id;
...
supabase/tests/smoke/t9_backdated_historical_traversal.sql:231:  delete from core_identity.org_node_versions where node_id = v_node_team_b_id;
supabase/tests/smoke/t9_backdated_historical_traversal.sql:232:  insert into core_identity.org_node_versions
supabase/tests/smoke/t9_backdated_historical_traversal.sql:265:  delete from core_identity.org_node_versions where node_id = v_node_team_a_id;
supabase/tests/smoke/t9_backdated_historical_traversal.sql:266:  insert into core_identity.org_node_versions
supabase/tests/smoke/t9_backdated_historical_traversal.sql:271:  delete from core_identity.employee_node_placements where employee_id = v_emp_a_id;
...
supabase/tests/smoke/t9_backdated_historical_traversal.sql:311:rollback;
supabase/tests/smoke/t9_backdated_historical_traversal.sql:9:begin;
supabase/tests/smoke/t9_backdated_historical_traversal.sql:32:  insert into core_identity.roles (name, description)
supabase/tests/smoke/t9_backdated_historical_traversal.sql:37:  insert into core_identity.employees (first_name, last_name, email, role_id)
supabase/tests/smoke/t9_backdated_historical_traversal.sql:45:  insert into core_identity.org_nodes (id) values (v_node_root_id),(v_node_dept_id),(v_node_team_a_id),(v_node_team_b_id);
...
exec
/bin/bash -lc "sed -n '120,210p' supabase/tests/smoke/t9_placements.sql && sed -n '160,285p' supabase/tests/smoke/t9_backdated_historical_traversal.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
exec
/bin/bash -lc "rg -n \"create table core_identity\\.role_permission_grants|role_permission_grants_select|grant select on table core_identity\\.role_permission_grants\" supabase/migrations/202605180000*.sql supabase/migrations/20260518100000_t9_fundament_supplement.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
supabase/migrations/20260518000006_t9_grants_and_helpers.sql:8:create table core_identity.role_permission_grants (
supabase/migrations/20260518000006_t9_grants_and_helpers.sql:42:grant select on table core_identity.role_permission_grants to authenticated;
supabase/migrations/20260518000006_t9_grants_and_helpers.sql:43:create policy role_permission_grants_select on core_identity.role_permission_grants
...
exec
/bin/bash -lc "sed -n '1,60p' supabase/migrations/20260518000006_t9_grants_and_helpers.sql" in /home/mathias/stork-2.0
 succeeded in 0ms:
...
codex
KRITISK — TEKNISK-BLOKERING: T10.6 `core_identity.clients_validate_fields()` introducerer en ny function-body reader med `cfd.is_active = true`, men T10.16 allowlister kun `core_identity.client_field_definitions_list`.

`legacy-is-active-readers` i `scripts/fitness.mjs` scanner live functions for `where/and ... is_active = true` uden `status = 'active'` og filtrerer kun `LEGACY_IS_ACTIVE_EXEMPT_FUNCTIONS`. `clients_validate_fields()` matcher regexen og vil ikke være allowlistet. Resultat: fitness/CI blokerer efter migration.

LØS: Udvid T10.16 med:

`core_identity.clients_validate_fields`

sammen med `core_identity.client_field_definitions_list`, og opdatér Fundament-tjek/Test-konsekvens til at nævne begge R7d-allowlist entries.

Code-svar forventet: ACCEPT.
tokens used
133,416
KRITISK — TEKNISK-BLOKERING: T10.6 `core_identity.clients_validate_fields()` introducerer en ny function-body reader med `cfd.is_active = true`, men T10.16 allowlister kun `core_identity.client_field_definitions_list`.
