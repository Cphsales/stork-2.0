...
Fallback-note: GitHub-connector write fik 403 (`Resource not accessible by integration`). Lokal `git pull`/commit er blokeret fordi `.git/index.lock` ikke kan oprettes paa read-only filesystem. Local HEAD matcher notification-commit `6ff77a2`, og tracker-issue #12 blev laest via GitHub-connector.
