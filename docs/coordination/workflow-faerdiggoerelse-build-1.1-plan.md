...
- **`scripts/kaede/dirigent.mjs`** — qwers-format-regler (l.62–72, dok-kommentar — IKKE selve aktiveringen), dispatch-log (l.34 `.dispatch-log.jsonl`; idempotens i `udfoer()` l.332+), fail-closed (l.48/70/253/353/395/448/525/615), transport-commit (l.359/425), parallel dispatch (l.180–184). **Bevares uændret som motor.** A1 tilføjer kun kobling: kaede-state ↔ `workflow/`-runtime-facit; **divergens → BLOKER**.
- **`scripts/kaede/tilstand.mjs`** (READ-ONLY state-loader, l.3) — den FAKTISKE divergens-/gate-fakta-mekanisme A1 udvider: `findDivergens` (l.60–66: flagger felter hvor kilder er uenige, `vaerdier.size>1`), `laesTilstand` (l.267+: normaliserer git/gh/fs til ét tilstands-objekt; remote-divergens l.276), `afledEvents` (l.102–110: **qwers-åbnings-afledning** — `ord.tekst.startsWith("qwers ")` → `qwers-aabning`-event, author-filtreret; samt gate-events `krav-ok-hash`/`gate-godkendt/afvist`). A1 tilføjer **`workflow/`-gate-state som kilde i `findDivergens`** → kaede↔workflow-divergens BLOKER. **Read-only-normaliser-og-flag-designet bevares 1:1.**
- **`scripts/kaede/kaede-regler.json`** — author-check (l.8–10: `gate_author:"mgrubak"`, `bot:"stork-code-bot"`) + **qwers-aabning-routing** (l.75–78: event → `code:recon-kode` + `codex:recon-research`). **Bevares.**
...
- **Gate-vokabular ligger reelt TRE steder og DIVERGERER i dag (Codex #173 r2):**
  - `workflow/gate-def.json` (l.7–13): `gateOrd.eksterne` = `krav OK` / `plan OK` / `build OK` → states `krav-laast`/`plan-laast`/`build-laast`; `_kontrakt` påstår selv "eksekverbar = sandheden". `workflow/regelflade.md` (l.9) + `workflow/worklog.json` (gate-state `kravOK`/`planOK`/`buildOK`) deler dette.
  - `scripts/kaede/kaede-regler.json` (l.11): `gate_ord` = `qwers` / `krav OK` / `qwerg` / `slut OK` / `GODKENDT` / `AFVIST` / `stop`.
  - `disciplin.md` (l.40/49/52): de to ubetingede Mathias-gates = `krav OK <hash>` + `slut OK`. **Ingen `plan OK`/`build OK`.**

  **Konkret divergens:** workflow/-substratet kører livscyklus `krav→plan→build`; den levende kæde + `disciplin.md` kører `krav→slut`. **A1/A7 må IKKE bevare denne divergens.** A1 reconciler de tre til **`disciplin.md` som ENESTE definitions-hjem** (`governance-owns`, disciplin.md:3); `gate-def.json` + `regelflade.md` + `kaede-regler.json` bliver **runtime-projektioner** der afledes af / skal matche disciplin.md (gate-def.json's "eksekverbar = sandheden"-selvpåstand nedgraderes til projektion). **Det kanoniske gate-ord-sæt (`krav→plan→build` vs `krav→slut`) er en Mathias-gate-lås ved A1** (jf. Fremgangsmåde: "Mathias låser gates sidst" — gate-ord er hans domæne). Indtil reconcilet: tre-vejs-divergens i gate-ord = **BLOKER** (ikke bevaret). A7 fodrer gates med runtime-artefakter EFTER reconcile; ændrer ikke selv definitionerne.
...
> **Scope-grænse (Mathias 2026-06-18):** fulde §3.1 **1:1 kode-bodies + diff** for A1/A2's flader (`dirigent.mjs` · `tilstand.mjs` · `kaede-regler.json` · `gate-def.json` · `regelflade.md` · `worklog.json` · læseflade · `disciplin.md`) leveres i **build-PR'erne A1/A2**, reviewet `--phase=build` mod reel kode. **IKKE waived** — de er merge-blockers dér. **Acceptance-mekanismen (A10) udskydes derimod IKKE** (konkret nedenfor): Build 1.1 findes netop for at stoppe falsk/strukturel grøn, så leverings-beviset må aldrig være en blank check.
...
- **`scripts/workflow/e2e-check.mjs` → omdøbes til `scripts/workflow/kontrakt-kompositions-check.mjs`** (det er **ikke** e2e — det tråder fixtures); tagges `structural` og **fjernes fra acceptance** (CI: flyttes til `workflow:selftest`, ud af acceptance-gaten). Den eneste reelle e2e / leverings-bevis er **A10-runneren** (`scripts/workflow/acceptance-run.mjs`, se A10) — strukturelle selftests kan støtte, men er **aldrig** leverings-beviset.
...
**Konkret acceptance-mekanisme (bindende — leverings-beviset, ikke strukturelt):**

> **Princip (Mathias 2026-06-18):** alt der giver grønt testes som **adfærd** på en reel committet pakke. Structural/selftests må findes og støtte, men er **aldrig** leverings-beviset. **Build 1.1 må ikke merge på strukturel eller syntetisk grøn.**

- **Runner:** `scripts/workflow/acceptance-run.mjs` (NY; **ikke** en `*-check.selftest.mjs`). Kører den reelle front-halvkæde (trigger → aktører → recon → krav → gates) på den committede pakke — ingen in-memory-stubs.
- **Kommando:** `npm run workflow:acceptance` → `node scripts/workflow/acceptance-run.mjs --gate`. **Adskilt** fra `workflow:selftest` (structural).
- **Committet testpakke (commit-isolation):** `workflow/acceptance-pakke/` — reel basis-pakke + F01–F26 seed-varianter, **committet** med indholds-hash. Runneren beregner selv `computedArtifactSha` fra de committede filer og binder hver `runtimeProof` til den. **Ingen pakke uden for git tæller.**
- **Fixture-forbud (bindende):** runneren **nægter** at køre mod fixtures / literals / placeholders / in-memory-stubs; tomt / `"x"` / genereret input → **FAIL** (antiPaperGreen udvidet til runneren). En `acceptance`-F-ID uden reel `runtimeProof` på den committede pakke = **rød gap**, vist åbent — ingen exit-0-rapport-snyd.
- **F01–F26 = negative kanariefugle:** hver F-ID er en **seedet fejl** kæden SKAL fange. Runneren kører seed-varianten gennem den reelle station og **observerer den faktiske disposition**: `runtimeProof = { fId, station, observedDisposition, computedArtifactSha, capturedAt }`. Fanger kæden IKKE en seedet fejl (kanariefuglen overlever) → **acceptance FAIL**.
- **runtimeProof-validering (maskinel):** gaten validerer (a) hver `acceptance`-F-ID har en `runtimeProof` bundet til pakkens `computedArtifactSha` (**ikke** selvrapporteret), (b) `observedDisposition === expectedFailure` (kolonnen "Forventet"), (c) **ingen `structural`-tagget test bidrager** til acceptance. Mismatch / manglende / stale-SHA → **FAIL/BLOKER**.
- **CI-gate:** nyt `acceptance`-job i `.github/workflows/ci.yml` kører `npm run workflow:acceptance` og **fejler buildet** ved enhver rød/manglende `runtimeProof` eller disposition-mismatch. CI adskiller eksplicit **structural** (`workflow:selftest`) fra **acceptance** (`workflow:acceptance`); **kun acceptance er leverings-bevis** (S14/M2-grøn).
...
- **Codex runde 4 på #173 (egen git-læsning, 2026-06-18):** §8.1 INGEN-MODSIGELSE; 2 fund (§3.1 fulde bodies; A10 manglede eksekverbar spor) → **V5** (Mathias-afgørelse: fuld plan-komplethed for A10 — falsk/strukturel grøn må aldrig være leverings-bevis): **A10/acceptance fuldt konkretiseret** — runner `scripts/workflow/acceptance-run.mjs`, `npm run workflow:acceptance --gate`, committet `workflow/acceptance-pakke/` + commit-isolation, fixture-forbud, maskinel `runtimeProof`-validering, F01–F26 som negative kanariefugle, CI-gate adskilt fra structural. §3.1 1:1 kode-bodies for A1/A2 → build-PR (**ikke waived**, merge-blockers `--phase=build`).
- **Codex runde 5 på #173 (egen git-læsning, 2026-06-18):** A10-konkretisering **accepteret** ✓; §8.1 INGEN-MODSIGELSE. Tilbage: (a) faktuel qwers-anker-fejl → **V5.1** rettet (qwers-afledning = `tilstand.mjs:102–110` + `kaede-regler.json:75–78`, ikke `dirigent.mjs`); (b) **§3.1 fulde-bodies-i-planen** — Codex afviser build-PR-afgrænsningen → **governance-altitude-konflikt, afventer Mathias** (fuld build-plan i #173 vs. §3.1 respekterer split-plan-altitude).
