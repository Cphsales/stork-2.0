...
- **`scripts/kaede/tilstand.mjs`** (READ-ONLY state-loader, l.3) — den FAKTISKE divergens-/gate-fakta-mekanisme A1 udvider: `findDivergens` (l.60–66: flagger felter hvor kilder er uenige, `vaerdier.size>1`), `laesTilstand` (l.267+: normaliserer git/gh/fs til ét tilstands-objekt; remote-divergens l.276), `afledEvents` (l.102+: afleder gate-events fra `gateOrd` filtreret på `gateAuthor` — `krav-ok-hash`, `gate-godkendt/afvist`). A1 tilføjer **`workflow/`-gate-state som kilde i `findDivergens`** → kaede↔workflow-divergens BLOKER. **Read-only-normaliser-og-flag-designet bevares 1:1.**
