...
**Dato:** 2026-06-10 · **Branch:** claude/gov-4-branch-protection-build + claude/gov-4-step5 · **Merge-commit:** `996d0d4` (rebase-HEAD, PR #112; build-PR #110 @ `dff836a`)
...
| (d) bot-PR + mgrubak-approval + grøn CI | **PR #112 selv** — første PR under fuld gate; (c)-beviset (#111 BLOCKED) viser kravet bider; (d) BEKRÆFTET: #112 merged med mgrubak APPROVED (API-verificeret; tidligere approval korrekt DISMISSED ved nyt push — dismiss-stale virkede). Første merge udført af Code under ny konvention. (#110 var bot-authored og merged af mgrubak, men FØR step 4 og uden formel approval — utilstrækkelig som (d), fund R8-1) |
