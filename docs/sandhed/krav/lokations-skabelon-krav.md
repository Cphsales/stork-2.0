...
Status: UDKAST · alle Mathias-svar 2026-09-02/03 indarbejdet · recon_oid: 7fbd3a2f43c03ce239aa81e20483326bf438463c @ commit afcf4080a6c3619462eb9e53df10f49495e19abf · anker: masterplan §4 trin 10b / §1.12
...
**HVAD:** En lokation kan have flere stande (placements) under sig — samme entitet, ét hierarki (Mathias 2026-09-03: "i en lokation kan oprettes flere stande"). Standen er den enhed én klient ad gangen står på: flere klienter kan være på lokationen samtidig, men aldrig to klienter på samme stand samtidig — låst forretningsregel. En stand kan bære egen dagspris; ellers gælder lokationens.

- **Struktur (umuligt):** cykler (en lokation kan aldrig — direkte eller gennem led — være sin egen over-lokation) · stand uden entydig pris-opløsning · to klienter på samme stand samtidig · booking på stand under ikke-aktiv lokation (via K-4) · stande der forsvinder ved nedlæggelse.
...
4. Én klient pr. stand ad gangen: skabelonen leverer standen som den enhed en booking binder til, så to klienter på samme stand på samme dag AFVISES i booking-leddet (trin 24 forbruger stand-identiteten). Tilladelsen (K-6) gives pr. lokation; hvilken stand klienten står på afgøres ved booking.
...
*Plan-fase:* Mathias' model er lokation → stande; om en stand kan have under-stande er ikke forretnings-afgjort (planner afgør i rammen) · gruppe-arv på stand — inden for dette kravs ramme.
...
4. Der findes INGEN kæde-tilladelse: en tilladelse (K-6) gives altid pr. lokation.
...
6. Tilladelser (K-6) MÅ registreres mens lokationen er i dvale — det AFVISES IKKE (den bookede dato skal ligge efter dvalens ophør; håndhævelsen af selve booking-datoen er trin 24).
...
8. At koble en klient på en nedlagt lokation AFVISES — lokationen skal være åben først (Mathias 2026-09-03: "den skal være åben", bekræftet "nej"); genåbn (ac 7), kobl derefter.
9. Nedlæggelse kobler automatisk alle klienter fra lokationen (K-6 ac 7) og beholder lokationens stande (K-2 ac 5); koblings-historikken består (Mathias 2026-09-03). Dvale kobler ingen af (ac 6).
...
- **Struktur (umuligt):** booking under hvile (via K-4) · en "book alligevel"-fravigelse · hvile-konfig-ændring uden rettighed eller uden audit-spor · periode-afslutning uden rettighed eller uden audit-spor.
- **Værdi (UI):** hvile-periodens længde/enhed pr. lokation ("periode styres i ui") · manuel dvale og afslutning før tid · hvem der må ændre (rettigheder, K-8).
...
**HVAD:** Stork styrer hvilke klienter der må stå og sælges på hvilke lokationer (forretningsforstaaelse §14) — systemet håndhæver det, aldrig en manuel konvention. Klienter kobles på og fra de enkelte lokationer (K-3; Mathias 2/9: pr. lokation, ikke pr. kæde · 2026-09-03: "klienter skal altid kunne kobles på og fra på de enkelte lokationer") — af rettighedshavere. En kobling gælder fra den sættes indtil klienten kobles fra, eller lokationen nedlægges — ved nedlæggelse kobles alle klienter automatisk fra (Mathias 2026-09-03), historikken består, standene består. Ingen slutdato kræves (Mathias 2026-09-03: "lokationer har ingen slutdato før vi deaktivere den/nedlægger den"). Dvale rører ikke koblingerne (K-4 ac 6); kun nedlæggelse kobler af. Flere klienter kan have tilladelse på samme lokation samtidig; grænsen er standen (K-2): én klient pr. stand ad gangen, og hvilken stand klienten står på afgøres ved booking (trin 24). Fortiden bevares: hvad der gjaldt hvornår kan altid ses (princip 9). Historik bevares altid (Mathias 2026-09-03: "vigtigt vi bevarer den historiske data"). *Plan-mekanik (ikke krav-sprog):* gældende-dato, versionering og fortrydelses-mekanismen (§1.7) følger fundamentets mønster for daterede ændringer.

- **Struktur (umuligt):** tilladelse uden klient eller lokation · to gældende tilladelser for samme klient×lokation (én sandhed) · kæde-tilladelse (K-3) · kobling på nedlagt lokation (K-4) · nedlagt lokation med koblede klienter · sletning/omskrivning af historik · booking af klient uden gældende tilladelse på lokationen på bookingdatoen (håndhævet i skrivevejen — trin 24 forbruger opslags-fladen).
- **Værdi (UI):** hvilke klienter er koblet på hvilken lokation, fra hvornår; frakobling; fortrydelses-periodens længde pr. ændrings-type.

**Acceptkriterier (slut-effekter):**
1. Tilladelse uden klient eller lokation AFVISES.
2. En anden gældende tilladelse for samme klient på samme lokation AFVISES — der er én sandhed om forholdet.
3. Ingen slutdato kræves: en kobling uden slutdato AFVISES IKKE; den gælder indtil klienten kobles fra eller lokationen nedlægges (Mathias 2026-09-03).
4. Frakobling lukker forholdet fra en dato og ændrer aldrig fortiden; sletning eller omskrivning af historik AFVISES.
5. Opslaget "må klient X stå på lokation Y på dato D?" svarer entydigt — også for historiske datoer (fladen trin 24's booking-gate forbruger).
6. Flere klienter kan have tilladelse på samme lokation samtidig — det AFVISES IKKE (Mathias 2026-09-03). Grænsen er standen: én klient pr. stand ad gangen (K-2 ac 4).
7. Nedlæggelse kobler automatisk alle klienter fra lokationen (Mathias 2026-09-03: "klienter kobles automatisk fra lokationer når den nedlægges") — som frakobling pr. dato, historikken består ("vigtigt vi bevarer den historiske data"); en nedlagt lokation med koblede klienter kan IKKE findes. Lokationen beholder sine stande (K-2 ac 5). Efter genåbning kobles klienter på igen (K-4 ac 8). Dvale kobler ingen af.
8. Til-/frakobling uden rettighed AFVISES; daterede ændringer får godkendelse + fortrydelses-periode før de træder i kraft.

*Trin 24 (noteret nedstrøms-afhængighed):* hvad der sker med allerede bookede dage når en klient kobles fra — bookingen forbruger tilladelses-opslaget pr. dato; konsekvensen for eksisterende bookinger afgøres i trin 24.
...
**HVAD:** Alle pakkens forretningshandlinger — oprette/redigere lokationer, stande, grupper, tilladelser; status-skift; hvile-styring; klassifikations-valg — kan udføres i UI af rettighedshavere. Skellet er hans: **struktur er hardkodet** (required-felter, forbud — fx at en lokation skal have et navn), **værdier styres i UI** (hvilket navn, hvilken klient, hvilke perioder).
...
| intet-data:hierarki-dybde-og-placement-regler | 3 | behandlet | K-2 (Mathias 2026-09-03: lokation → flere stande, én klient pr. stand ad gangen; dybde under stand + gruppe-arv → plan-fase, synligt flyttet) |
| intet-data:klient-tilladelse-kommerciel-betydning | 3 | behandlet | K-6 (Mathias 2/9: pr. lokation, gruppe ejer — K-3 · 3/9: flere klienter pr. lokation, én pr. stand · kobles på/fra pr. lokation, ingen slutdato · nedlæggelse kobler automatisk alle af, stande + historik består · kobling på nedlagt afvises, åben først (K-4 ac 8) · eksisterende bookinger ved frakobling → trin 24) |
