...
Status: UDKAST (runde 2, efter fresh-eyes-audit 2c6e8f6) · alle Mathias-svar 2026-09-02/03 indarbejdet · 2 afledninger med citat (K-1: lokation uden klienter, M-12/M-14 · K-5 ac 3: straks-virkning, M-29.5) — fremlægges som bekræftelses-linjer ("står medmindre du siger stop", jf. M-32: afled-før-spørg), blokerer ikke upload · kilder pr. K citerer ledger `plan-build/lokations-skabelon/mathias-ord.md` (M-1..M-34 @ 17c69a3, ledger-blob 6d41d2161fd2) · recon_oid: 7fbd3a2f43c03ce239aa81e20483326bf438463c @ commit afcf4080a6c3619462eb9e53df10f49495e19abf · anker: masterplan §4 trin 10b / §1.12
...
**Kilder:** M-23 ("alt skal kunne styres i ui. Dermed skal det være muligt at oprette en lokaition i ui") · M-24 ("en loaktion skal have et navn og en client. men hvilkvet navn og hvilken client styres i ui" — navn = krævet felt; "client" læses som klient-dimensionen via gruppen (K-3/K-6), IKKE som et krav om mindst én klient pr. lokation — *afledt med citat: M-12 "klienter skal altid kunne kobles på og fra" → den sidste kan kobles fra, så lokationen kan stå uden klienter; M-14: ved nedlæggelse kobles alle af og lokationen består. Fremlægges som bekræftelses-linje, blokerer ikke upload*) · §1.12 (lokations-entitetens felter, type-listen, status, hvile, anonymized_at) · forretningsforståelse §3 + §14 (attribution via klient — "ingen parallel attribution") · §2.7.8 ("Ikke en særmodel … Booking-flow er identisk uanset lokations-type") · forretningsforståelse §14 (lokation som master-data).
...
- **Værdi (UI):** hvilke grupper, navne, typer og øvrige felter (om feltlisten er fast eller UI-udvidelig via felt-registry afgøres i plan-fasen — klient-mønstret er analogi, ikke krav) · hvilke klienter er koblet på gruppen · hvilke klienter en lokation fravælger; kontaktperson-oplysninger er persondata (K-7).
...
3. Ændring af antal hviledage uden rettighed AFVISES; med rettighed auditeres den med årsag. *Afledt med citat: M-29.5 ("styres i retigheder") → rettighed = direkte handling med audit; ændringen gælder straks, uden godkendelses-trin. Fremlægges som bekræftelses-linje (står medmindre Mathias siger stop), blokerer ikke upload. Materialitet: auto-hvile udløses først i trin 24. Princip 5's kladde→testet→godkendt→aktiv læses som gældende for data-håndterings-konfiguration, ikke for hvile-perioden; siger Mathias stop, følger ændringen fortrydelses-mekanismen (K-6 ac 10).*
...
**Kilder:** M-21 ("kan hvis antal hvile dage er valgt i ui") · M-25.2 ("periode styres i ui"; M-7 = parafrase heraf, afløst) · M-27b ("nej, men man skal kunne stoppe dvaleperioden og dermed åbne lokationen i ui" — reglen absolut, perioden stoppes i UI) · M-29.5 ("styres i retigheder" — afledt: straks-virkning uden godkendelses-trin; bekræftelses-linje i fremlæggelsen) · §1.12 + Appendix A (cooldown pr. lokation, ikke pr. klient) · §2.7.1 (cooldown-trigger ved booking — trin 24) · princip 4 (default = intet).
...
**HVAD:** Hver ny data-kolonne i pakken klassificeres (kategori, persondata-niveau, opbevaring) — default er intet (princip 4). Lokations-rækken og dens forretningsbærende historik bevares (§11-grænsen: entiteten bevares); klassifikationen vælges pr. felt (ac 4-mønstret) med konservativ default — recon flager at fx en adresse kan være indirekte persondata for en enkelt-butik. *Deklareret som vores læsning; intet Mathias-ord klassificerer lokationens felter.* Gruppens kontaktperson-oplysninger er persondata med sletteregler (§11: samme grænse som kontaktperson på klienten) og skal kunne anonymiseres uden at gruppen eller dens lokationer mistes. Lokationen bærer anonymized_at som doc-låst struktur (§1.12).
...
- **Værdi (UI):** klassifikations-valgene pr. felt (persondata-niveau, opbevaring) · hvilke kontakt-felter gruppen har (feltliste fast eller UI-udvidelig → plan-fase, analogi til klient-mønstret).
...
Hvordan mindst én stand sikres ved oprettelse/sletning (K-2 — under-stande findes ikke, M-18) · gruppe-arv på stand (K-2/K-3) · om den effektive ret klient×lokation afledes eller materialiseres, og om en lokations fravalg består gennem nedlæggelse og genåbning samt om fravalgs-/frakoblings-handlinger på en nedlagt lokation afvises (default: afvises, som til-valg i K-4 ac 8) (K-6) · om gruppe og §1.12's leverandør er én eller to entiteter, om type er krævet ved oprettelse (default: valgfri ved oprettelse, krævet før rabat-brug i trin 29), og hvordan en gruppe tages ud af brug (K-3) · anonymiserings-vej for gruppens kontakt-felter — rigtige kolonner vs. registry-jsonb (jsonb walkes ikke af den generiske anonymisering; K-7 ac 4's dæknings-gate fanger valget) (K-7) · valuta-/enheds-repræsentation og tom-pris-gyldighed (K-1) · om gruppens feltliste er fast eller UI-udvidelig og dens udformning, CVR m.v. — analogi-kilde: klient-mønstret (masterplan client_field_definitions); default: som for klienter (K-3) · seeding-detaljer ud over superadmin (K-8) · fortrydelses-wiring: ændrings-typer og undo-defaults (K-6/K-8) · om dvale-status og hvile-mekanisme er én eller to model-ting (K-4/K-5) · direkte-vs-godkendelses-mekanik pr. handlings-type inden for UI-rammen (K-8).
...
| doc:docs/strategi/stork-2-0-master-plan.md#1.12:klient-tilladelser-form | 2 | behandlet | K-6 (relations-tabel klient×lokation = plan-mekanik; forretnings-modellen præciseret af M-17: kobling på gruppen, fravalg pr. lokation — vedligeholds-flag, se "Holdt mod låste docs") |
...
| intet-data:konfig-lifecycle-for-cooldown | 3 | behandlet | K-5 (M-29.5: "styres i retigheder" — rettighed + audit; straks-virkning uden godkendelses-trin — afledt, bekræftelses-linje i fremlæggelsen) |
| intet-data:leverandoer-entitetens-fulde-indhold | 3 | behandlet | K-3 (feltliste fast/UI-udvidelig → plan-fase m. klient-analogi som default; rabataftale → trin 29) |
| intet-data:lokations-pii-og-retention | 3 | behandlet | K-7 (§11-grænsen: lokations-rækken bevares; felt-klassifikation pr. felt m. konservativ default — vores læsning; gruppe-kontakt = persondata) |
